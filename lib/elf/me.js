...
class Me {
  ...
  get desktop() {
    ...
    return {
      id: this.winDesktopId(),
      emit: async (topic, data) =>
        quest.evt.send(`greathall::<${await this.winDesktopId()}-event>`, {
          topic,
          data,
        }),
    };
  ...
  get cryo() {
    ...
    return {
      ...
      listenTo: async (actorType, callback) => {
        ...
        quest.evt.full(triggerId + '-updated');
      },
    };
  }
  ...
  get on() {
    const quest = this.#quest;
    return (eventName, listener, fromActorId = null) => {
      if (/<>(::)/.test(eventName)) {
        throw new Error('eventName: characters <, > and :: are reserved');
      }
      eventName = `*::${fromActorId || '*'}.<${eventName}>`;
      quest.goblin.defer(
        quest.sub.local(
          eventName,
          isAsync
            ? async (_, {msg}) => await listener(msg?.data)
            : (_, {msg}) => listener / msg?.data
        )
      );
      return this;
    };
  ...
  emit(eventName, data, actorId = null) {
    ...
    eventName = `${actorId || this.id}.<${eventName}>`;
    this.#quest.evt.full(eventName, data);
  }

  async newQuestFeed(prefix) {
    if (`${this.id}.create` === this.#quest.calledFrom) {
      throw new Error('newQuestFeed cannot be called from a create quest');
    }
  ...
  async winDesktopId() {
    ...
    const desktopIds = new Set(
      [
        this.#quest.msg.data?.desktopId /* Provided by the quest call */,
        this.#quest.goblin.feed /*         Provided by the actor      */,
      ].filter((desktopId) => !!desktopId)
    );

    /* Look at local windows */
    for (const desktopId of desktopIds) {
      const windowId = WM.getWindowIdFromDesktopId(desktopId);
      if (windowId) {
        return desktopId;
      }
    }

    /* Fallback on the warehouse (maybe the "window" is remote) */
    const labs = await this.#quest.warehouse.query({
      type: ['laboratory', 'carnotzet'],
      view: ['feed'],
    });
    const feeds = labs.reduce((feeds, {feed}) => feeds.add(feed), new Set());

    /* Check if a desktopId matches with one lab */
    for (const desktopId of desktopIds) {
      if (feeds.has(desktopId)) {
        return desktopId;
      }
    }

    throw new Error(
      `No window available, winDesktopId cannot be used here\n→ desktopIds: ${[
        ...desktopIds,
      ].join(', ')}`
    );
  }
