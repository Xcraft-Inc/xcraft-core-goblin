...
class Me {
  ...
  async winDesktopId() {
    ...
    for (const desktopId of desktopIds) {
      /* If desktopId exists as actor (branch), it's a good candidate because
       * it's not the case with system desktopId.
       */
      const subs = await this.#quest.warehouse.getBranchSubscriptions({
        branch: desktopId,
      });
      if (subs.length) {
        return desktopId;
