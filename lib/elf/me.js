...
const logicTraps = {
  apply(target, self, args) {
    const params = self._cacheReduceParams.get(
    ...
    const payload = params
      .filter(
        (key, index) =>
          self._quest.msg?.data?.[key] &&
          self._quest.msg?.data?.[key] !== args[index]
      )
      .reduce((payload, key, index) => {
...
class Me {
  ...
  constructor(quest, cacheReduceParams) {
    this.#quest = quest;
    this.#cacheReduceParams = cacheReduceParams;
    ...
    props.forEach((prop) => {
      ...
      if (prop === 'logic' && quest.elf.logic) {
        if (!this.logic) {
          this.logic = {
            _quest: quest,
            _cacheReduceParams: this.#cacheReduceParams,
          };
        }
  ...
  get cryo() {
    return {
      ...
      getState: async (...args) =>
        await cryoManager.getState(this.#quest, ...args),
      getIds: async (...args) => await cryoManager.getIds(this.#quest, ...args),
      queryLastActions: async (...args) =>
        await cryoManager.queryLastActions(this.#quest, ...args),
      isPersisted: async (...args) =>
        await cryoManager.isPersisted(this.#quest, ...args),
  ...
  get persist() {
    return async (...args) => {
      ...
      if (goblinConfig.actionsSync?.enable) {
        this.#sync.sync(this._ripley.persist.db, this.state.id);
      }
    };
  }
