...
class Me {
  ...
  async listenTo(elfType, callback) {
    const cryoAPI = this.#quest.getAPI('cryo');
    const triggerId = `${this.id}.<${this.id}-${elfType}-listenTo>`;
    const triggerArgs = {
      actorType: elfType,
    ...
    this.quest.goblin.defer(
      this.#quest.sub.local(
        `*::${triggerId}-*`,
        async ({msg}) => await callback(msg.topic, msg.data)
      )
    );
    this.quest.goblin.defer(
    ...
    this.#quest.evt(`<${this.id}-listenTo-changed>`);
  }

  async newQuestFeed() {
    const feedId = Me.createFeed();
