...
const hordesSync = require('./hordesSync.js');
...
class Me {
  ...
  constructor(quest) {
    ...
    if (goblinConfig.enableActionsSync && this?._ripley?.persist) {
      this.#sync = debounce(
        () => hordesSync().sync(),
        500
      ); /* fire and forget */
  ...
  get cryo() {
    return {
      ...
      extractBlob: async (...args) =>
        await cryoManager.extractBlob(this.#quest, ...args),
      getState: async (...args) =>
      ...
      syncDB: async (db) => await hordesSync().syncDB(db),
    };
  }
  ...
  get persist() {
    return async (...args) => {
      ...
      if (goblinConfig.enableActionsSync) {
        hordesSync().set(this._ripley.persist.db, this.state.id);
        this.#sync();
      }
    };
  }
