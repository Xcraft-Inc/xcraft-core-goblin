...
class Elf {
      ...
      if (logic instanceof Elf.Archetype) {
        ...
        Goblin.registerQuest(
          ...
          async (quest, actions, commitId) => {
             ...
             * 'create' action (or at least one persist). If it's true, then
             * we replay the received 'create' action. In all other cases,
             * the received 'create' actions must be ignored.
             */
            const cryo = quest.getAPI('cryo');
            const createCnt = await cryo.countCreateOrPersist({db, goblin});
            const skipCreate = createCnt > 1;
   ...
   *
   * @example
   * const instance = new Service(this).insertOrCreate(id, dekstopId, state);
   * if (instance) { //
   ...
   */
  async insertOrCreate(id, desktopId, state) {}
