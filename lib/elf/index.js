...
const directTraps = {
  async apply(target, self, args) {
    ...
    if (isAsync(target)) {
      ...
      if (
        (name === 'create' || name === 'init' || name === 'delete') &&
        ...
        if (
          !me._ripley ||
          ((name === 'create' || name === 'init') && !me._elfStateLoaded)
        ) {
  ...
  async apply(target, self, args) {
    ...
    if (name === 'create') {
      ...
      if (self._elfSession) {
        ...
        _args.desktopId = self._elfSession.desktopId;
        _args.parent = self._elfSession.parent;
      }
    ...
    if (logicClass) {
      ...
      if (logic instanceof Elf.Archetype) {
        ...
        goblinConfig.ripley = utils
          ...
          .filter((name) => name !== 'create' && name !== 'init')
          .reduce(
            (ripley, logicName) => ({
              ...ripley,
              ...{
