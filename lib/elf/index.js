...
class Elf {
  ...
  static configure(elfClass, logicClass) {
     ...
     * That's indices useful with the JSON actions in order to improve
  ...
  get cryo() {
    return {
       ...
       * @param {string} db database name
       * @param {string} vectors a vectors MATCH query
       * @param {string} locale selected locale partition
       * @param {string} scope scoped to results
       * @param {number} [limit] max results
       * @returns {Promise<Iterable<object>>}
       */
      searchDistance2: async (db, vectors, locale, scope, limit = 100) => {},
      /**
