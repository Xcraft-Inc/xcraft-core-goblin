...
const {string, number, object, option} = require('xcraft-core-stones');
      ...
      if (logic instanceof Elf.Archetype) {
        ...
        Goblin.registerQuest(
          ...
          async (quest, actions, commitId) => {
            ...
            // await quest.me.persist(null, commitId);
            const action = {
              type: 'persist',
              meta: {commitId},
              payload: quest.goblin.getState().toJS(),
            };
            await cryo.freeze({
              db,
              action,
              rules: {goblin, mode: 'all'},
              raw: false,
            });
 ...
 * try {
 *
 *   await new Elf(this).create(id, feedId); //create elfs on this feed
 *
 * }
 * finally {
 *
 *   this.killFeed(feedId); //kill all created elfs
 *
 * }
 *
