...
const xLog = require('xcraft-core-log')('elf');
...
class Elf {
  ...
  static configure(elfClass, logicClass) {
    ...
    quests.forEach((name) => {
      ...
      if (name !== 'delete' && !isAsync(elfClass.prototype[name])) {
        xLog.warn(
          `The "${name}" quest of "${goblinName}" should be async even if it seems useless. Quests declared as generators or simple functions are deprecated (excepted for the "delete" quest that should be sync).`
        );
      }
    ...
    if (logicClass?.indices?.length) {
      configured.handlers._postload = function (msg, resp) {
        try {
          ...
          cryo._setIndices(logicClass.db, logicClass.indices, goblinName);
  ...
  static async $create(id, desktopId, ...args) {
    ...
    const populateWithDefaults = (state) =>
      Array.from(this.state._state.keys())
        .filter((key) => !Object.hasOwn(state, key))
        .forEach(
          (key) =>
            (state[key] = this.state[key]?.toJS
              ? this.state[key].toJS()
              : this.state[key])
        );

    if (state) {
      populateWithDefaults(state);

      if (process.env.GOBLIN_CHECKTYPE) {
        const shape = this.quest.elf.logic.state.getShape();
        const check = checkType(state, shape);
        if (!check.ok) {
      ...
      if (insertOrReplace) {
        ...
        /* Revert Ripley pending (without persist) staged actions on create */
        const revert = async () => {
          const goblin = `${id.split('@', 1)[0]}-${id}`;
          const db = this._ripley.persist.db;
          const cryo = this.quest.getAPI('cryo');
          await cryo.revert({db, goblin});
        };

        await revert();
        this.quest.goblin.defer(async () => await revert());
      }
      this._stateLoaded = true;
    } else if (desktopId === 'system@ripley') {
      // Put an initial state if actor is created by ripley
  ...
  async create(id, desktopId, ...args) {
    return await Elf.$create.call(this, id, desktopId, ...args);
  }
  ...
  /**
   * Callback for on registering.
   * @callback OnListener
   * @param {any} data data
   * @returns {Promise<void> | void}
   */
  /**
   * Register a listener on this actor for an event.
   *
   * The listener is unsubscribed when the actor is deleted. It's not an usual
   * event emitter API. This API is an abstraction based on the Quest.sub
   * functions.
   *
   * The third argument can be used to listen only for a specific actor id.
   * It's mostly useful when you want to delegate work in your actor.
   * @example
   * // Listen for event 'something-done' when coming from all possible actors
   * // the internal topic looks like: `*::*.<something-done>`
   * this.on('something-done', async (data) => {});
   * // Listen for event 'something-done' when coming from my actor
   * // the internal topic looks like: `*::myActor@id.<something-done>`
   * this.on('something-done', async (data) => {}, this.id);
   * // Chaining is supported
   * this
   *   .on('something-started', (data) => {}, this.id)
   *   .on('something-stopped', (data) => {}, this.id);
   * @param {string} eventName Event's name
   * @param {OnListener} listener
   * @param {string | null} [fromActorId] default is from all actors
   * @returns {this} this
   */
  on(eventName, listener, fromActorId = null) {
    return this;
  }
   ...
   * Emit an event.
   *
   * It's not an usual event emitter API. This API is an abstraction based on
   * the Quest.evt functions.
   *
   * The third argument can be used to send with a specific actor id instead
   * of this.id. It provides a way to a specific subscriber to catch only these
   * events.
   *
   * For example when an actor 'first' subscribe with its id, the 'second'
   * actor can ensures that the events are provided for 'first'. A 'third'
   * one can still catch all events.
   * @example
   * // Actor 'first'
   * this.on('something-done', () => {}, this.id); // this.id is 'first@main'
   * // Actor 'second'
   * this.emit('something-done', {}, 'first@main');
   * // Actor 'third'
   * this.on('something-done', () => {}); // catch all, use the [*] joker
   * @example
   * // Send an event (it uses this.id)
   * this.emit('something-done', {});
   * // Send an event by specifying an other actor
   * this.emit('something-done', {}, 'manager@main');
   * @param {string} eventName Event's name
   * @param {object | Shredder} data
   * @param {string | null} [actorId]
   */
  emit(eventName, data, actorId = null) {}
  ...
  /**
   * Retrieve the desktopId suitable for a window (local or remote)
   * @protected
   * @returns {Promise<`${string}@${string}`>} the desktopId
   */
  async winDesktopId() {
    return '';
  }
  ...
  get cryo() {
    return {
       ...
       * @param {string} db database name
       * @param {string[]} patterns raw string in the action
       * @param {RegExp} regex extractor for the action strings
       * @param {object} [options]
       * @param {boolean} [options.last] use lastPersistedActions table
       * @returns {AsyncIterable<Promise<{id: string, refs: string[]}>>}
       */
      searchRaw: async function* (db, patterns, regex, options = null) {},
      /**
       ...
       * @returns {Promise<Iterable<string>>}
       */
      getIds: async (db, pattern, options = null) => {},
      /**
       * @param {string} db database name
       ...
       * @param {`${T}@${string}` | null} id actor's id
       */
      /**
       ...
       * @template {string} T
       * @param {T} actorType
       * @param {ListenToCallback<T>} callback
       */
...
class Archetype extends Elf.Spirit {
  /**
   * Test if an actor exists in the database (even if this actor is trashed)
   * @param {*} cryo
...
function SculptElf(type) {
  ...
  type;
  function SculptedState(_) {
    const obj = Object.assign(this, _);
    return obj;
  }
  SculptedState.prototype.getShape = () => type;
  return SculptedState;
}
