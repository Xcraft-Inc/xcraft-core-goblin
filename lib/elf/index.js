...
class Elf {
  ...
  static configure(elfClass, logicClass) {
    ...
    /* Provide user indices to Cryo and (or) history disabling feature.
     * Tese indices are useful with the JSON actions in order to improve
     * a lot the queryArchetype performances.
     * The 'no history' option is useful in order to keep only the last
     * commited persist action (a bit like a squash).
     */
    if (logicClass?.indices?.length || logicClass?.noHistory) {
      configured.handlers._postload = function (msg, resp) {
        try {
          ...
          if (logicClass?.noHistory) {
            cryo._setNoHistory(logicClass.db, goblinName);
          }
  ...
  static trial(logicClass) {
    ...
    _logic.state = Elf.#proxyfyLogicState(_logic.state);
