...
const forwardTraps = {
  async apply(target, self, args) {
    ...
    if (name === 'create') {
      ...
      let state;
      if (!self._bus?.rpc) {
        state = await self.quest.getState(self.id);
      }

      Object.defineProperty(api, 'state', {
        get() {
          if (!state && self._bus?.rpc) {
            throw new Error(
              `Automatic state loading is disabled in case of Xcraft RPC "create"`
            );
          }
          if (!api._state) {
            api._state = Elf._proxyfyLogicState(self.state);
            api._state._state = state;
            api._state.toJS = state.toJS.bind(api.state._state);
          }
          return api._state;
        },
      });
