...
const logicTraps = {
  apply(target, self, args) {
    ...
    if (!params) {
      throw new Error(
        `You cannot call a logic reducer without a corresponding quest, see "${target.name}" of "${self._quest.goblin.goblinName}"`
      );
    }
...
function injectState(Elf, self, api) {
  Object.defineProperty(api, 'state', {
    get() {
      let state = null;
      if (!self._bus?.rpc) {
        state = self.quest._getLocalState(self.id);
      }
      if (!state) {
        throw new Error(
          `Automatic state loading is disabled in case of remote "create" (by RPC or tribes)`
        );
      }
      if (!api._state) {
        api._state = Elf._proxyfyLogicState(self.state);
      }
      api._state._state = state;
      return api._state;
    },
  });
  return api;
...
const forwardTraps = {
  async apply(target, self, args) {
    ...
    if (name === 'api') {
      ...
      api._elf = true;
      return injectState(Elf, self, api);
    }
    ...
    if (
      ...
      if (name === 'create' && params[1] !== 'desktopId') {
        self.quest.log.err(
          `Missing desktopId as second parameter of ${goblinName}.${name}`
        );
      }
