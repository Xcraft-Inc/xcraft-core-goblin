...
class Quest {
  ...
  isAlive(goblinId) {
    ...
    if (Goblins.has(namespace) && Goblins.get(namespace).has(goblinId)) {
      const goblin = Goblins.get(namespace).get(goblinId);
      return goblin.isCreated() ? true : false;
    }
    return false;
  }
  ...
  _sub(resp, topic, handler) {
    const {isAsync, isGenerator} = require('xcraft-core-utils/lib/js.js');
...
class SubAPI {
  ...
  *_callAndWait(call, resp, respArgs, topic, next) {
    const {isAsync, isGenerator} = require('xcraft-core-utils/lib/js');
