...
function newResponse({moduleName, orcName, caller, questName, user}) {
  ...
  _resp.cmd = watt(function* (cmd, args, next) {
    ...
    //GUILD: check moduleName
    if (args && args._goblinUser) {
      user = args._goblinUser;
    }
    return yield Quest._cmd(_resp, cmd, args, caller, questName, user, next);
  });
...
class Quest {
  ...
  *sysKill() {
  ...
  static *_cmd(resp, cmd, args, caller, questName, goblinUser, next) {
