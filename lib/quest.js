    ...
    .forEach((item) => {
      ...
      API_REGISTRY[namespace][item.call] = (cmd, LAZY_API) =>
        watt(function* (payload) {
          let _payload = arguments.length < 2 ? {} : payload;

          if (LAZY_API._elf) {
            const params = Goblin.getParams(namespace, item.call);
            _payload = params.reduce((obj, key, index) => {
              /* The last argument is always the watt's next callback */
...
function newResponse({
  ...
  _resp.cmd = watt(function* (cmd, args, next) {
    ...
    return yield Quest._cmd(_resp, cmd, args, caller, questName, desktopId);
  });
...
class Quest {
  constructor(context, name, msg, resp) {
    ...
    this.sub.passive = subAPI.passive.bind(subAPI);
  ...
  *createView(namespace, args, view) {
  ...
  *createEntity(id, properties, view, next) {
    if (!next) {
      next = view;
      view = null;
    }
  ...
  *getState(goblinId) {
    ...
    return yield this.cmd(`goblin-registry.${routingKey}.getState`, {goblinId});
  }
  ...
  static async _cmd(resp, cmd, args, caller, questName, desktopId) {
    ...
    const msg = await resp.command.nestedSendAsync(cmd, args);
    return msg.data;
  }
  ...
  *cmd(cmd, args, next) {
  ...
  _sub(resp, topic, handler, xcraftRPC = false) {
    let _isWatt = false;
    let _isAsync = false;
  ...
  *_kill(args, next) {
    ...
    for (const id of ids) {
      ...
      this.warehouse.detachFromParents(payload, next.parallel());
    }
    yield next.sync();
  }
  ...
  *kill(ids, parents, feed, next) {
    ...
    return yield this._kill({ids, parents, feed}, next);
  }
...
class SubAPI {
  ...
  passive(topic, handler) {
    const respArgs = this.quest.respArgs();
    const resp = newResponse(respArgs);
    return this.quest._sub(resp, topic, handler, true);
  }
