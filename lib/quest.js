      ...
      API_REGISTRY[namespace][item.call] = (cmd, LAZY_API) =>
        watt(function* (payload) {
          ...
          if (LAZY_API._elf) {
            ...
            if (params === false) {
              /* It's the server side case, all parameters must be required */
              params =
                COMMANDS_REGISTRY[`${namespace}.${item.call}`].options.params
                  .required;
            }
...
class Quest {
  ...
  *getState(goblinId) {
  ...
  static *_cmd(resp, cmd, args, caller, questName, desktopId, next) {
