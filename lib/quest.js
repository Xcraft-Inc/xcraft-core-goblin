...
function newResponse({moduleName, orcName, transports, caller, questName}) {
  const _resp = busClient.newResponse(moduleName, orcName, transports);
...
class Quest {
  ...
  respArgs() {
    return {
      ...
      transports: this._resp.transports,
  ...
  *createFor(
    ...
    if (args && args._goblinFeed) {
      ...
      if (Goblins.has(ownerName) && Goblins.get(ownerName).has(goblinId)) {
        feed = Goblins.get(ownerName).get(goblinId).feed;
      } else {
