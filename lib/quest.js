...
const subMethods = {
  ...
  'evt.full': function (topic, payload) {
    return Quest._evt(this.resp, topic, null, payload);
  },
  'sub.local': function (topic, handler) {
    const respArgs = this.respArgsLocal();
    const resp = newResponse(respArgs);
    return this._sub(resp, topic, handler);
  },
  'sub._wait': watt(function* (resp, respArgs, topic, next) {
  ...
  'sub.wait': watt(function* (topic) {
    const respArgs = this.respArgs();
    return yield this.sub._wait(this.resp, respArgs, topic);
  }),
  'sub.localWait': watt(function* (topic) {
    const respArgs = this.respArgsLocal();
    const resp = newResponse(respArgs);
    return yield this.sub._wait(resp, respArgs, topic);
  }),
...
class Quest {
  ...
  *createCache(goblinId) {
    ...
    const Goblins = this.Goblin.getGoblinsRegistry();
    if (
  ...
  *sysCall(questName, questArguments) {
    const namespace = this.Goblin.getGoblinName(this.goblin.id);
  ...
  *sysKill() {
  ...
  getSystemDesktop() {
    return this.Goblin.getSystemDesktop(this.getDesktop());
  }

  getDesktop(canFail) {
    return this.Goblin.getDesktop(this.msg, this.goblin, canFail);
  }
  ...
  getAPI(id, namespace, withPrivate, autoSysCall) {
    ...
    if (!namespace) {
      namespace = this.Goblin.getGoblinName(id);
    }
  ...
  getState(goblinId) {
    const Goblins = this.Goblin.getGoblinsRegistry();
    const namespace = this.Goblin.getGoblinName(goblinId);
  ...
  *cmd(cmd, args, next) {
  ...
  static _evt(resp, topic, caller, payload) {
    if (!payload) {
      payload = {};
    }
    ...
    resp.events.send(caller ? `${caller}.${topic}` : topic, payload);
  }
