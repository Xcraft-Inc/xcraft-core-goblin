...
const subMethods = {
  ...
  'sub.wait': watt(function*(topic, next) {
    const moduleName = this.goblin.goblinName;
    const orcName = this._resp.orcName;
    const transports = this._resp.transports;

    const _next = next.parallel();
    const unsubWait = this.resp.events.subscribe(topic, msg => {
      const resp = busClient.newResponse(moduleName, orcName, transports);
...
class Quest {
  ...
  *cmd(cmd, args, next) {
    ...
    if (args && typeof args === 'object') {
      ...
      args._goblinCaller = this.goblin.id;
      args._goblinCallerQuest = this._questName;
    }
