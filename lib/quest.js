...
class Quest {
  ...
  *cmd(cmd, args, next) {
    ...
    if (Goblin.isBlocked(this.goblin, cmd)) {
      throw new Error(
        `Blocked by a shield, ${this.goblin.id} is not allowed to run ${cmd}`
      );
    }
    if (Goblin.isBlocked(this.user, cmd)) {
      throw new Error(
        `Blocked by a shield, ${this.user.id} is not allowed to run ${cmd}`
      );
    }
