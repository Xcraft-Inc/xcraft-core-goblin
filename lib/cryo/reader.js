...
class CryoReader extends SQLite {
  ...
  *getGoblinIds(goblinType, options) {
    ...
    const type = options?.type || 'persist';
    const last = options?.last || false;
    const stmt = last
      ? this.stmts(this._dbName).getLastGoblinIds.bind({
          goblin: `${goblinType}-*`,
        })
      : this.stmts(this._dbName).getGoblinIds.bind({
          goblin: `${goblinType}-*`,
          type,
        });

    for (const {goblinId} of stmt.iterate()) {
      yield goblinId;
    }
  }
