...
function sqlCondition(condition) {
  if (condition.operator === 'eq') {
    return `${sqlPath(condition.path)} = ${escape(condition.value)}`;
  }
  ...
  if (condition.operator === 'and') {
    return condition.conditions.map((cond) => sqlCondition(cond)).join(' AND ');
  }
  if (condition.operator === 'or') {
    return condition.conditions.map((cond) => sqlCondition(cond)).join(' OR ');
  }
...
class CryoReader extends SQLite {
  ...
  *queryLastActions(quest, goblinType, properties, filters) {
    ...
    WHERE goblin LIKE $goblinType AND ${sqlCondition(filters)}`;

    const statement = this.prepare(this._dbName, sql);
    for (const res of statement.iterate({goblinType})) {
      yield res;
    }
  }
