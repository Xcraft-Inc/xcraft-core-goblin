...
class CryoReader extends SQLite {
  ...
  attachReader(reader) {
    ...
    if (!reader._open(reader._dbName)) {
      return () => null;
    }
  ...
  _attachDB(name, attachPath) {
    const handle = this._db[name];
    if (CryoReader.#attach.is(handle, name)) {
      CryoReader.#attach.in(handle, name);
      return () => this._detachDB(name);
    }
    try {
      this.stmts(this._dbName).attach.run({attachPath, name});
      xLog.info(`database '${name}' attached to '${this._dbName}'`);
      CryoReader.#attach.in(handle, name);
      return () => this._detachDB(name);
    } catch (err) {
      xLog.err(err);
  ...
  _detachDB(name) {
    const handle = this._db[name];
    if (CryoReader.#attach.is(handle, name)) {
      CryoReader.#attach.out(handle, name);
      if (!CryoReader.#attach.is(handle, name)) {
        this.stmts(this._dbName).detach.run({name});
        xLog.info(`db '${name}' detached from '${this._dbName}'`);
      }
    }
  }
