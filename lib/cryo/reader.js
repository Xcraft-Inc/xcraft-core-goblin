...
class CryoReader extends SQLite {
  ...
  *getGoblinIds(goblinType, options) {
   ...
   * be true in order to preventuseless attach/detach since it will be handled
   * by the worker.
   * @example
   * const query = reader.queryArchetype('foo', FooShape, true).select(...);
   * const {stream} = await cmsReader.queryWorkerStream(query.json());
   * const results = [];
   *
   * for await (const chunk of stream) {
   *   const rows = JSON.parse(chunk);
   *   results.push(...rows.map((row) => query.mapRow(row)));
   * }
   * @param {{db: string, sql: string, values: any[] | null, attachs: any[]}} query
   * @returns {Promise<{stream: MessagePortReadable, routingKey: string}>}
   */
