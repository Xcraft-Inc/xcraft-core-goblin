...
class CryoManager {
  ...
  async #tryCryoReader(quest, db) {
    ...
    return new CryoReader(location, db, getHandle);
  }

  async #tryCryoSearch(quest, db) {
    const {getHandle, location} = await this.#try(quest, db);
    return new CryoSearch(location, db, getHandle);
  }
  ...
  async *searchRaw(quest, db, pattern, regex) {
    const fullTextSearcher = await this.fullTextSearcher(quest, db);
    return fullTextSearcher
      ? yield* fullTextSearcher.searchRaw(pattern, regex)
      : yield [];
  }
  ...
  async getIds(quest, db, goblinPattern, type) {
    const reader = await this.reader(quest, db);
    return reader ? reader.getGoblinIds(goblinPattern, type) : [];
  }
