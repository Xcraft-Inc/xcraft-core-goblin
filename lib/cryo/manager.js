...
class CryoManager {
  ...
  async *searchRaw(quest, db, pattern, regex, options = null) {
    const fullTextSearcher = await this.fullTextSearcher(quest, db);
    return fullTextSearcher
      ? yield* fullTextSearcher.searchRaw(pattern, regex, options)
      : yield [];
  }
  ...
  async getIds(quest, db, goblinType, options = null) {
    const reader = await this.reader(quest, db);
