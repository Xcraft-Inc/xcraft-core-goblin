...
async function applyPersisted(quest, db, horde, actions) {
  const cryo = quest.getAPI('cryo');

  quest.log.dbg(`ripley for ${db}, begin with ${actions.length}`);
  ...
  sendSyncing(quest, db, horde, true, {pos: 0, max: 0});
}
...
class RipleyWriter extends Writable {
  ...
  async #computeStepsAndApply(persisted) {
    ...
    for (
      ...
      await applyPersisted(this.#quest, this.#db, this.#horde, actions);
