...
module.exports.buildRemoteGuestFootprint = (clientServiceId, ctx) => {
  const {ip, socketId} = ctx;
  return `guest@passenger@${ip}@${clientServiceId}@${socketId}`;
};
...
module.exports.identifyUser = (msg) => {
  ...
  if (msg.data._goblinUser) {
    try {
      ...
      identified._goblinUser = msg.data._goblinUser;
    } catch {
      identified = null;
    }
  }
  ...
  if (!identified) {
    ...
    const footprint = Goblin.buildGuestFootprint(`system@${appId}`, '0');
