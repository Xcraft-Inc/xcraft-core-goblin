...
class Goblin {
  ...
  *upserter(quest, payload) {
    ...
    payload.data = new Shredder({id});

    let upserted = false;
    if (!this.lastUpsertData) {
      this.lastUpsertData = data;
      upserted = yield quest.warehouse.upsert(payload);
    } else if (!this.lastUpsertData.equals(data)) {
      upserted = yield quest.warehouse.upsert(payload);
    }

    if (upserted) {
      const store = quest.getAPI('warehouse-store');
      yield store.upsert({branch: payload.branch, data});
      this.lastUpsertData = data;
    }
  }
