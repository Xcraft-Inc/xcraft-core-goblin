...
class Goblin {
  ...
  static updateTTL(goblinId, TTL) {
    const goblinTTL = GOBLINS['goblin-ttl'][`goblin-ttl@${process.pid}`];

    const timeout = watt(function*(next) {
      goblinTTL.store.dispatch({
        type: 'update',
        goblinId,
        delay: 0,
        timeout: null,
      ...
      yield busClient.command.send(
        ...
        {
          branch: goblinId,
          parents: [`goblin-ttl@${process.pid}`],
        },
        null,
        next
      );
        ...
        if (
          ...
          if (quest.hasDispatched) {
            if (
              ...
              yield Goblin.rank(this._goblinName, this._goblinId);
            }
