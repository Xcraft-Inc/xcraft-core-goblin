    ...
    Object.keys(QUESTS[goblinName]).forEach(questName => {
      ...
      if (questName === 'create') {
        quests[questName] = watt(function*(msg, resp, next) {
          ...
          if (createMutex[id]) {
            const unsub = resp.events.subscribe(
              `${goblinName}.${questName}.${msg.id}.(error|finished)`,
              () => {
                unsub();
  ...
  injectQuestBusHelpers(quest, resp) {
    ...
    quest.createFor = watt(function*(
      ...
      const id = yield quest.cmd(
        `${namespace}.create`,
        Object.assign({createdBy: goblinId}, args)
      );
      ...
      try {
        ...
        if (!errThrown) {
          ...
          if (questName === 'create' && !canceled) {
            const unsub = resp.events.subscribe(
              `*::warehouse.${this._goblinId}.released`,
              () => {
                unsub();
                resp.command.send(`${this.goblinName}.delete`, {
                  id: this._goblinId,
                });
              }
            );
