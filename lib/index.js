...
class Goblin {
  ...
  static release(goblinName, goblinId) {
    const delCond = GOBLINS[goblinName][goblinId].delCond;
  ...
  doQuest(questName, msg, resp, createMutex) {
    return watt(function*(quest, next) {
      ...
      let errThrown = false;
      let canceled = false;
      const delCond = GOBLINS[this._goblinName][this._goblinId].delCond;
      try {
        this.getState().attachLogger(resp.log);

        if (questName !== 'delete') {
          if (delCond) {
            delCond.wait(deleting => !deleting, next.parallel());
            yield next.sync();
          }
        }
        ...
        if (questName === 'delete') {
          ...
          GOBLINS[this._goblinName][
            this._goblinId
          ].delCond = _locks.createCondVariable(true);
