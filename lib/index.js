...
class Goblin {
  ...
  static release(goblinName, goblinId) {
    GOBLINS[goblinName][goblinId].dispose();
  ...
  constructor(
    ...
    this._serieRunner = $('serie', queue)
      ...
      .nfcall([])
      .sequence()
      .stopOnError(err => console.warn(err.stack || err));

    this._parallelRunner = $('parallel', queue)
      .map(({goblin, questName, quest, dispatch, done, doneId}) => next => {
        Goblin._questDispatch(
          goblin,
          questName,
    ...
    this._createRunner = $('create', queue)
      .map(({goblin, questName, quest, dispatch, done, doneId}) => next => {
        const _done = (...args) => {
          if (questName === 'create') {
