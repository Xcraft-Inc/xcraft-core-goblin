...
const {isGenerator, isFunction} = js;
...
class Goblin {
  ...
  static registerSafeQuest(goblinName, questName, questFunc) {
    Goblin.registerQuest(goblinName, questName, function* (quest, $msg, next) {
    ...
    Object.keys(QUESTS[goblinName]).forEach((questName) => {
      ...
      if (questName === 'create') {
        quests[questName] = watt(function* (msg, resp, next) {
          ...
          let id;
          try {
            if (!msg.data._goblinLegacy) {
              throw new Error(
                `sending command ${questName} manually is forbidden, use quest.create`
              );
            }
            ...
            if (!isAttached) {
              throw {
                code: 'SILENT_HILL',
                message: 'attach impossible (parent unavailable in the feed)',
              };
            }
      ...
      quests[questName] = watt(function* (msg, resp, next) {
        ...
        try {
          ...
          if (questName === 'delete' && !msg.data._goblinLegacy) {
            throw new Error(
              `sending command ${questName} manually is forbidden`
            );
          }
          ...
          //attach
          if (questName !== 'delete') {
            const desktopFeed = Goblin.getDesktop(msg, goblin, false);
            if (!desktopFeed.startsWith('system')) {
              const {data: isAttached} = yield resp.command.nestedSend(
                `warehouse.attach-to-parents`,
                {
                  branch: goblin.id,
                  parents: ['goblin'],
                  feeds: [Goblin.getSystemDesktop(desktopFeed)],
                },
                next
              );

              if (isAttached) {
                msg.data._goblinAttachedToSys = true;
              }
            }
          }
  ...
  static getDesktop(msg, goblin, canFail) {
    //desktopId in msg.data have priority
    let d = msg.data && msg.data.desktopId;
    if (!d) {
      //we can look in instance in fallback case... but it's not recommanded...
      d = goblin.getX('desktopId');
      if (!d) {
        if (!canFail) {
          throw new Error(`unable to get desktop id in ${goblin.id}`);
        }
      }
    }
    return d;
  ...
  deferCreateEvent(orcName, msgId) {
    this._createMsgIdList.push({orcName, msgId});
  }
  ...
  *_doQuest(questName, msg, resp, context, next) {
    ...
    if (msg.data._goblinRecreate) {
      ...
      if (this._runningQuests.create > 1) {
        this.deferCreateEvent(msg.orcName, msg.id);
        return;
      }
    ...
    try {
      ...
      //detach when needed
      if (msg.data._goblinAttachedToSys) {
        delete msg.data._goblinAttachedToSys;
        yield quest.warehouse.detachFromParents({
          branch: this._goblinId,
          parents: ['goblin'],
          feed: quest.getSystemDesktop(),
        });
      }

      if (questName === 'create') {
        this._createMsgIdList.push({orcName: msg.orcName, msgId: msg.id});
      }
      ...
      if (!errThrown) {
        ...
        try {
          if (questName === 'create') {
            ...
            this._createMsgIdList.forEach(({orcName, msgId}) =>
              resp.events.send(
