...
const guildEnforcer = require('./guildEnforcer.js')(coreGoblinConfig);
  ...
  if (action.type === 'DOASYNCQUEST') {
    ...
    if (isBlocked(goblin, cmd)) {
      throw new Goblin.ShieldedError(
        `Blocked by a shield, ${goblin.id} is not allowed to run ${cmd}`
      );
    }
    const user = Goblin.identifyUser(action.msg);
    if (isBlocked(user, cmd)) {
      throw new Goblin.ShieldedError(
        `Blocked by a shield, ${user.id} is not allowed to run ${cmd}`
      );
    }
...
class Goblin {
  ...
  static registerAliasNamespace(alias, namespace) {
...
module.exports.buildGuestFootprint = (clientServiceId) => {
  const os = require('os');
  const guestHost = os.hostname().toLowerCase().replace(/@/g, '-');
  const guestUser = os.userInfo().username.toLowerCase().replace(/@/g, '-');
  return `${guestUser}@${guestHost}@${clientServiceId}`;
};

//add guild member from JWT token
module.exports.enroleUser = (instance, tokenData) => {
  if (guildEnforcer.isBlocked(instance, guildEnforcer)) {
    throw new Error('enroleUser failed');
  }
  guildEnforcer.enroleUser(tokenData);
};

module.exports.identifyUser = (msg) => {
  if (msg.data && msg.data._goblinUser) {
    //identified case
    const [local, domain] = msg.data._goblinUser.split('@', 2);
    const login = `${local}@${domain}`;
    if (guildEnforcer.users[login]) {
      return guildEnforcer.users[login];
    }
  }
  //guest case fallback
  const {appId} = require('xcraft-core-host');
  const footprint = Goblin.buildGuestFootprint(`system@${appId}`);
  if (!guildEnforcer.users[footprint]) {
    guildEnforcer.addGuestUser(footprint);
  }
  return guildEnforcer.users[footprint];
};
