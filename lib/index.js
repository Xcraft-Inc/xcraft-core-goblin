...
class Goblin {
  ...
  injectQuestBusHelpers(quest, resp) {
    ...
    quest.createFor = watt(function*(
      ...
      if (args.id && GOBLINS_REFS[args.id]) {
        if (GOBLINS_REFS[args.id].count > 0) {
          skipCreate = true;
          console.log('Skip creation of ', args.id);
        }
      }
    ...
    return watt(function*(quest, next) {
      ...
      try {
        ...
        if (
          ...
          !GOBLINS_REFS[this._goblinId].created
        ) {
        ...
        if (!errThrown) {
          ...
          if (questName === 'create' && !canceled) {
            GOBLINS_REFS[this._goblinId].created = true;
