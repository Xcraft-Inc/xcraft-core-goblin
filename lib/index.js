...
class Goblin {
  ...
  static getQuests(goblinName) {
    ...
    const notifyCreated = (resp, id, msg) => {
    ...
    Object.keys(QUESTS[goblinName]).forEach(questName => {
        ...
        quests[questName] = watt(function*(msg, resp, next) {
          try {
            ...
            if (
              !exit &&
              GOBLINS[goblinName] &&
              GOBLINS[goblinName][msg.data.id]
            ) {
      ...
      quests[questName] = watt(function*(msg, resp) {
        try {
          ...
          if (GOBLINS[goblinName][goblinName]) {
            ...
            if (questName === 'delete') {
              yield locks.getMutex.lock(msg.data.id);
            }
  ...
  injectQuestBusHelpers(quest, resp) {
    ...
    quest.release = (goblinId, orcName) => {
    ...
    return watt(function*(quest, next) {
      ...
      try {
        ...
        if (questName === 'create' || questName === 'delete') {
          locks.getMutex.unlock(this._goblinId);
          if (questName === 'create') {
            quest.evt('created');
          } else {
            quest.evt('deleted');
