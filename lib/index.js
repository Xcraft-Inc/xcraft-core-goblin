...
class Goblin {
      ...
      if (questName === 'create') {
        quests[questName] = watt(function*(msg, resp, next) {
          try {
            ...
            const isSingleton = id === goblinName;

            if (!isSingleton && id.indexOf('@') === -1) {
              throw new Error(
  ...
  static create(goblinName, uniqueIdentifier) {
    ...
    if (GOBLINS[goblinName][goblinName]) {
      ...
      goblinId = uniqueIdentifier || `${goblinName}@${uuidV4()}`;
    }
  ...
  doQuest(questName, msg, resp, createMutex) {
    return watt(function*(quest, next) {
      ...
      try {
        ...
        if (
          !isSingleton ||
          (isSingleton && questName !== 'create' && questName !== 'delete')
        ) {
        ...
        if (questName === 'create' && (errThrown || canceled) && !isSingleton) {
