    ...
    Object.keys(QUESTS[goblinName]).forEach(questName => {
      ...
      if (questName === 'create') {
        quests[questName] = watt(function*(msg, resp, next) {
          ...
          if (GOBLINS[goblinName] && GOBLINS[goblinName][msg.data.id]) {
            setTimeout(() => resp.command.retry(msg), 50);
            return;
          }
      ...
      quests[questName] = (msg, resp) => {
        ...
        if (!msg.data) {
          resp.events.send(
            `${goblinName}.${questName}.${msg.id}.error`,
            new Error(`No id provided for ${goblinName}.${questName}`)
          );
          return;
        }
        if (!msg.data.id) {
        ...
        if (!goblin) {
          resp.events.send(
            `${goblinName}.${questName}.${msg.id}.error`,
            new Error(`Bad id ${msg.data.id} for ${goblinName}.${questName}`)
          );
          return;
        }
    ...
    return watt(function*(quest, next) {
      ...
      try {
        ...
        if (questName === 'create' && (errThrown || canceled)) {
           ...
           * delete its instance.
           */
          yield quest.cmd('warehouse.delete', {branch: this._goblinId});
