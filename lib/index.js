...
class Goblin {
  ...
  constructor (
    ...
    if (persistenceConfig) {
      ...
      for (const k in persistenceConfig) {
        this._persistenceConfig[k] = {};
        this._persistenceConfig[k].mode = persistenceConfig[k].mode;
        this._persistenceConfig[k].keys = persistenceConfig[k].keys;
      }
    }

    for (const k in this._persistenceConfig) {
      this._persistenceConfig[k].db = ripleyName;

      if (!('mode' in this._persistenceConfig[k])) {
        throw new Error (`Bad goblin persistence config, missing for ${k}`);
      }
      if (!this._persistence.hasMode (this._persistenceConfig[k].mode)) {
    ...
    if (this.usePersistence) {
      this._unsubscribePersistence = this._store.subscribe (() => {
        ...
        this._persistence.saveState (state.ellen.get (ripleyName));
      });
    }
  }
  ...
  doQuest (questName, msg, resp) {
    return watt (function* (quest, next) {
      ...
      if (!GOBLINS_REFS[this._goblinId] || GOBLINS_REFS[this._goblinId] === 0) {
        // FINISHED
        resp.events.send (
      ...
      if (questName === 'create') {
        ...
        if (GOBLINS_REFS[this._goblinId] > 1) {
          for (const g in uses) {
            const use = uses[g];
            GOBLINS_REFS[use.id]++;
          }
      ...
      try {
        ...
        if (questName === 'create' && GOBLINS_REFS[this._goblinId] === 1) {
        ...
        if (questName === 'delete' && GOBLINS_REFS[this._goblinId] === 0) {
          //console.log ('RUNNING DELETE QUEST:', this._goblinId);
          QUESTS[this._goblinName][questName] (quest, msg);
        }
        ...
        if (this.goblinName !== 'warehouse' && questName !== 'delete') {
        ...
        if (questName === 'delete') {
          if (GOBLINS_REFS[this._goblinId] === 0) {
            ...
            //Free stack maps...
            // FIXME delete SESSIONS[this.goblinName][this._goblinId];
          ...
          const dominos = dGob => {
            ...
            if (GOBLINS_REFS[dGob.id] >= 1) {
              GOBLINS_REFS[dGob.id]--;
            }

            if (GOBLINS_REFS[dGob.id] === 0) {
          ...
          Object.keys (postRelease).map (k => {
            ...
            //Free stack maps...
            // FIXME delete SESSIONS[dGob.goblinName][dGob.goblinId];
            ...
            const dQuest = {};
            const dMsg = {data: {id: dGob._goblinId}};
            dQuest.goblin = dGob;
