    ...
    quest.openInventory = () => {
      ...
      Object.keys (GOBLINS_USES).map (goblin =>
        Object.keys (GOBLINS_USES[goblin]).map (id =>
          Object.keys (GOBLINS_USES[goblin][id]).map (useKey =>
            inventory.push (GOBLINS_USES[goblin][id][useKey])
          )
        )
      );
      return {
        items: inventory,
        find: namespace =>
          inventory.find (use => use.namespace.startsWith (namespace)),
        use: namespace => {
          const use = inventory.find (use =>
            use.namespace.startsWith (namespace)
          );
          if (use) {
