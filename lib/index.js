...
module.exports.isBlocked = guildEnforcer.isBlocked;
...
module.exports.identifyUser = (msg) => {
  if (msg.data && msg.data._goblinUser) {
    ...
    if (!guildEnforcer.users[login]) {
      //use guest
      return guildEnforcer.users[msg.data._goblinUser];
    }
    return guildEnforcer.users[login];
  } else {
    //guest case
    const footprint = Goblin.buildGuestFootprint(msg.orcName);
