...
class Goblin {
  ...
  static questDispatch(goblin, questName, quest, dispatch, done, next) {
    const _questDispatcher = function(type, payload = {}, error = false) {
      const action = createAction(type, payload, error);
      dispatch(action);
    };
    goblin.running(questName, true);
    quest({dispatch: _questDispatcher, goblin}, err => {
