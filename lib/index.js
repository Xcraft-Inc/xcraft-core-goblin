...
$('quest', questEmitter).each(quest => {
  ...
  if (quest.questName === 'delete') {
    ...
    console.log(
      `running ${quest.goblin.id}.${quest.questName} in mode ${quest.cmdMode}`
    );
    ...
    $('dispatch-quest', this.questEmitter)
      .map(({goblin, questName, quest, dispatch, cmdMode}) =>
        watt(function*() {
          switch (cmdMode) {
            case 'delete':
              console.log(goblin.id, ' flushing...', cmdMode);
              yield Promise.all(createPromises);
              createPromises = [];
              yield Promise.all(apiPromises);
              apiPromises = [];
              console.log(goblin.id, ' flushing... [DONE]', cmdMode);
              nextParallel(
                done =>
                  queue.emit('parallel', {
                    goblin,
