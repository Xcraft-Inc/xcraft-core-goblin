...
class Goblin {
  ...
  constructor(goblinId, goblinName, logicState, logicHandlers, ripleyConfig) {
    ...
    if (goblinId === 'warehouse' && process.env.NODE_ENV !== 'production') {
    ...
    watt.wrapAll(this);
    this.upsert = _.debounce(this.upserter, 100);
  }
  ...
  *upserter(quest, payload) {
  ...
  injectQuestBusHelpers(quest, resp, msg) {
    ...
    quest.kill = watt(function*(ids, owners, next) {
      if (arguments.length === 2) {
        next = owners;
        owners = null;
      }
    ...
    quest.createFor = watt(function*(
      ...
      if (!args.desktopId) {
        args.desktopId = quest.getDesktop(true);
      }
    ...
    quest.isCanceled = result => result && result._QUEST_CANCELED_;
    ...
    quest.fail = (title, desc, hint, ex) => {
      ...
      const desktop = quest.getDesktop(true);
      if (desktop) {
