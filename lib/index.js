...
class Goblin {
  ...
  static configure(
    ...
    if (!RANKEDCACHE[goblinName]) {
      RANKEDCACHE[goblinName] = new RankedCache(
        (cacheConfig && cacheConfig.cacheSize) || 100
      );
  ...
  static create(goblinName, uniqueIdentifier) {
    ...
    GOBLINS[goblinName][goblinId] = new Goblin(
      ...
      CONFIGS[goblinName].cacheConfig
    );
