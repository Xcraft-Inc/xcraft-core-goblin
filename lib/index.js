...
class Goblin {
  ...
  static getQuests(goblinName) {
    ...
    Object.keys(QUESTS[goblinName]).forEach((questName) => {
      //Handle create
      if (questName === 'create') {
        quests[questName] = async function (msg, resp) {
          if (!msg.data) {
            msg.data = {
          ...
          try {
            ...
            const {data: isAttached} = await resp.command.nestedSendAsync(
  ...
  static getAllRipleyDB() {
    ...
    for (const config of Object.values(CONFIGS)) {
      if (config?.goblinConfig?.ripley?.persist) {
        dbs.add(config.goblinConfig.ripley.persist.db);
      }
    }
  ...
  static #createElf(goblin) {
    ...
    goblin._elfInstance = new CONFIGS[goblinName].goblinConfig.class(
      null,
      null,
      CONFIGS[goblinName].goblinConfig.ripley
    );
