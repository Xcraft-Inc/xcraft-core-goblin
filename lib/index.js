...
class Goblin {
  ...
  static #getReflect() {
    if (!Goblin.#reflect) {
      Goblin.#reflect = require('xcraft-core-utils/lib/reflect.js');
    }
    return Goblin.#reflect;
  }

  static #getHost() {
    if (!Goblin.#xHost) {
      Goblin.#xHost = require('xcraft-core-host');
    }
    return Goblin.#xHost;
  }

  static #getCryoManager() {
    if (!Goblin.#cryoManager) {
      Goblin.#cryoManager = require('./cryo/manager.js');
    }
    return Goblin.#cryoManager;
  }

  static #getCache() {
    if (!this.#cache) {
      this.#cache = require('./cache/index.js');
    }
    return this.#cache;
  }
  ...
  static registerQuest(goblinName, questName, quest, options) {
    ...
    const reflect = Goblin.#getReflect();
    ...
    if (opt.elf) {
      ...
      params = reflect.funcParams(quest);
     ...
     */
    const _quest = (q, m, n) => {
      const args = QUESTSMETA[goblinName][questName].params.reduce(
        (args, key) => {
          const arg = m.get(key);
          if (Array.isArray(arg) && key.startsWith('...')) {
            args.push(...arg);
          } else {
            args.push(arg);
          }
    ...
    return async (msg, resp) => {
      ...
      try {
        ...
        if (goblin && questName === 'insertOrReplace') {
          ...
          if (isServerSide) {
            const cryoManager = Goblin.#getCryoManager();
            cryoManager.syncBroadcast(rules.db);
          } else {
            const sync = Goblin.#getSync()();
            sync.sync(rules.db);
          }
        }
        ...
        if (!goblin) {
          ...
          let freeze = true;
          const quest = new Quest({goblin: {}}, questName, msg, resp);
          const hasGoblin = await cryoManager.isPersisted(
            quest,
            ripleyRules.db,
            id
          );
  ...
  static getActorClass(actorId) {
    const [actorType] = actorId.split('@', 1);
    const config = CONFIGS[actorType];
    return config?.goblinConfig?.class;
  }
   ...
   * @returns {string[]}
   */
  static getAllRipleyActors(db) {
    const types = [];
    for (const [type, config] of Object.entries(CONFIGS)) {
      if (db && config.goblinConfig.ripley.persist.db !== db) {
        continue;
      }
      if (config?.goblinConfig?.ripley?.persist) {
        types.push(type);
