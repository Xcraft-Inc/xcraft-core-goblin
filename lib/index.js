  ...
  static #insertQuest(goblinName, questName) {
    ...
    const cryoFreeze = async (resp, rules, action) => {
      await resp.command.sendAsync('cryo.freeze', {
        ...
        action,
        rules,
        raw: typeof action.meta.action === 'string',
      });
    };
    ...
    return async (msg, resp) => {
      ...
      try {
        ...
        if (goblin && questName === 'insertOrReplace') {
          ...
          if (!isServerSide) {
            const action = getAction(questName, id, msg.data.state);
        ...
        if (!goblin) {
          ...
          if (freeze) {
            ...
            if (!hasGoblin && !isServerSide && !commitId) {
              const action = getAction(questName, id, msg.data.state);
              await cryoFreeze(resp, rules, action);
            }
            ...
            const action = getAction('persist', id, msg.data.state, commitId);
