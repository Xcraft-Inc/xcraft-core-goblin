...
const guildEnforcer = require('./guildEnforcer.js')(
  busConfig,
  coreGoblinConfig
);
...
class Goblin {
  ...
  static getQuests(goblinName) {
    const quests = {};
    const cryoLocks = locks.getMutex;
    ...
    Object.keys(QUESTS[goblinName]).forEach((questName) => {
      ...
      if (questName === 'create') {
        quests[questName] = watt(function* (msg, resp, next) {
          ...
          try {
            ...
            /* lock for the special quickCreate quest */
            yield cryoLocks.lock(id);
            ...
            /* unlock for the special quickCreate quest */
            cryoLocks.unlock(id);
          }
        });
        return;
      ...
      if (questName === 'quickCreate') {
        quests[questName] = async (msg, resp) => {
          ...
          try {
            ...
            /* lock for the special quickCreate quest */
            await cryoLocks.lock(id);

            const goblin = GOBLINS.get(goblinName).get(msg.data.id);
            if (!goblin) {
              const cmd = promisify(resp.command.nestedSend).bind(resp.command);

              /* Inject the state in the Cryo database */
              const rules = Goblin.getActorRipleyRules(id, 'persist');
              rules.goblin = `${goblinName}-${id}`;
              const action = createAction('persist', {state: msg.data.state});

              /* freeze this persist even if a cryo-transaction is open /!\ */
              await cmd('cryo.freeze', {db: rules.db, action, rules});
            }
