...
setInterval(() => {
  console.log('FOREGROUND-QUESTS: ', foregroundParallelQuests);
  console.log('BACKGROUND-QUESTS: ', backgroundParallelQuests);
}, 5000);
const doAsyncQuest = watt(function*(quest, dispatch, goblin, scheduled) {
  ...
  if (scheduled) {
    ...
    switch (goblin._goblinConfig.schedulingMode) {
      default:
      case 'foreground':
        try {
        ...
        break;
      case 'background':
        setTimeout(() => doAsyncQuest(quest, dispatch, goblin, true), 1);
        break;
    }
  }
});
          ...
          try {
            ...
            const TTL =
              goblin && goblin._rankItem && goblin._rankItem.list
                ? 'Infinity'
                : msg.data._goblinTTL || (goblin && goblin.TTL);
            ...
            if (goblin && existRes.data === true) {
              ...
              delete createMutex[id]; /* Release for the next */
              return;
            }
    ...
    return watt(function*(context, next) {
      ...
      try {
        ...
        if (
          ...
          if (this._goblinConfig && this._goblinConfig.cacheSize > 0) {
