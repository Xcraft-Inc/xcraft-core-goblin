...
class Goblin {
  ...
  static createSingle(goblinName) {
    ...
    if (appArgs.tribe && appArgs.tribe > 0 && goblinName !== 'goblin') {
      console.log(
        `Singleton creation skipped for ${goblinName} (tribe: `,
        appArgs.tribe,
        `)`
      );
      return null;
  ...
  static registerAliasNamespace(alias, namespace) {
    if (ALIAS[alias]) {
      if (ALIAS[alias] !== namespace) {
        throw new Error(
          `Register alias conflict detected: ${alias}->${namespace}, existing: ${alias}->${ALIAS[alias]}`
        );
      }
    }
  ...
  deferCreateEvent(orcName, msgId, routing) {
    this._createMsgIdList.push({orcName, msgId, routing});
  }
  ...
  *_doQuest(questName, msg, resp, context, next) {
    ...
    if (msg.data._goblinRecreate) {
      ...
      if (this._runningQuests.create > 1) {
        this.deferCreateEvent(msg.orcName, msg.id, resp.events.routing);
        return;
      }
        ...
        try {
          if (questName === 'create') {
            ...
            for (let i = this._createMsgIdList.length - 1; i >= 0; --i) {
              const {orcName, msgId, routing} = this._createMsgIdList[i];
              resp.events.send(
...
module.exports.buildGuestFootprint = (clientServiceId) => {
  ...
  return `guest@${guestUser}@${guestHost}@${clientServiceId}`;
};
...
module.exports.identifyUser = (msg) => {
  let identified;
  try {
    const [userId, local, domain] = msg.data._goblinUser.split('@', 3);
    const login = `${local}@${domain}`;
    const user = guildEnforcer.users[login];
    if (user) {
      if (user.id === userId) {
        identified = user;
      }
    } else {
      if (!guildEnforcer.users[msg.data._goblinUser]) {
        guildEnforcer.addGuestUser(msg.data._goblinUser);
      }
      identified = guildEnforcer.users[msg.data._goblinUser];
    }
  } catch {
