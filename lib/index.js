...
const serialQuest = ['create', 'init', 'boot', 'delete'];
$('quest', questEmitter).each(quest => {
  if (quest.caller && quest.callerQuest) {
    //do create?
    if (serialQuest.includes(quest.questName)) {
      //already created ?
      if (quest.goblin.isCreating()) {
        if (quest.caller === quest.goblin.id) {
          quest.goblin.questEmitter.emit('run-priority', quest);
        } else {
          quest.goblin.questEmitter.emit('run-parallel', quest);
        }
      } else {
        //create or recreate
        quest.goblin.questEmitter.emit('run-create', quest);
      }
    } else {
      quest.goblin.questEmitter.emit('run-parallel', quest);
    }
...
const doAsyncQuest = watt(function*(
  quest,
  dispatch,
  questName,
  goblin,
  ...
  if (scheduled) {
    ...
    switch (goblin.schedulingMode) {
      ...
      case 'background':
        ...
        yield doAsyncQuest(quest, dispatch, questName, goblin, true);
        break;
    }
  }
});
...
class Goblin {
  ...
  static getQuests(goblinName) {
    ...
    Object.keys(QUESTS[goblinName]).forEach(questName => {
      ...
      const questDelete = (msg, resp, goblin) => {
        if (
          !msg.data._goblinMutexRecursive &&
          (goblinMutex[msg.data.id] || goblin.isRunning())
        ) {
      ...
      quests[questName] = (msg, resp) => {
        try {
          ...
          if (!goblin) {
            throw new Error(
              `Error calling quest ${goblinName}.${questName}: goblin with id ${
                msg.data.id
              } has not been created`
            );
          }

          if (questName === 'delete' && !questDelete(msg, resp, goblin)) {
            return;
          }

          // FIXME: it's commented because it seems that it breaks the cmd
          //        calls order. If this code is enabled, it's no longer
          //        possible to create a new Board in Polypheme for example.
          //        Other bug are possible...
          //        We must consider to ensure the same command order in any
          //        cases; with a retry, it's not possible to be safe.
          //        See https://github.com/Xcraft-Inc/xcraft-core-goblin/issues/4
          // if (
          //   goblin.isCreating() &&
          //   msg.data &&
          //   msg.data._goblinCaller !== goblin.id &&
          //   !['boot', 'init', 'create'].includes(msg.data._goblinCallerQuest)
          // ) {
          //   setTimeout(() => resp.command.retry(msg), 20);
          //   return;
          // }
  ...
  constructor(goblinId, goblinName, logicState, logicHandlers, goblinConfig) {
    ...
    const parallelRuns = $('run-parallel', this.questEmitter)
      .tap(({goblin, questName}) => {
        console.log(`running ${goblin.id}.${questName} in parallel`);
      })
      .map(({goblin, questName, quest, dispatch, schedulingMode}) => n => {
        const questDispatcher = function(type, payload = {}, error = false) {
          const action = createAction(type, payload, error);
          dispatch(action);
        };
        ...
        quest({dispatch: questDispatcher, goblin}, err => {
          goblin.running(questName, false);
          n(err, {goblin, questName});
        });
      })
      .nfcall([])
      .parallel(Number.MAX_VALUE)
      .tap(({goblin, questName}) =>
        console.log(`running ${goblin.id}.${questName} in parallel [DONE]`)
      )
      .stopOnError(err => console.warn(err));

    const apiRuns = parallelRuns.fork();

    $('run-priority', this.questEmitter)
      .tap(({goblin, questName}) => {
        console.log(`running ${goblin.id}.${questName} during creation`);
      })
      .map(({goblin, questName, quest, dispatch, schedulingMode}) => n => {
        const questDispatcher = function(type, payload = {}, error = false) {
          const action = createAction(type, payload, error);
      ...
      .parallel(Number.MAX_VALUE)
      .tap(({goblin, questName}) =>
        console.log(`running ${goblin.id}.${questName} during creation [DONE]`)
      )
      .stopOnError(err => console.warn(err))
      .done(() => console.log('ended'));

    $('run-create', this.questEmitter)
      .tap(({goblin, questName}) => {
        console.log(`CREATING ${goblin.id}.${questName}`);
      })
      .map(({goblin, questName, quest, dispatch, schedulingMode}) => n => {
        const questDispatcher = function(type, payload = {}, error = false) {
          const action = createAction(type, payload, error);
      ...
      .series()
      .tap(({goblin, questName}) =>
        console.log(`CREATING ${goblin.id}.${questName} [DONE]`)
      )
      .stopOnError(err => console.warn(err))
      .done(() => console.log('ended'));
      ...
      try {
        ...
        if (
          ...
          if (questName === 'create' || isSingleton) {
            payload.parent = parent || this._goblinId;
