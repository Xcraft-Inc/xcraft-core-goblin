...
class Goblin {
  ...
  injectQuestBusHelpers (quest, resp) {
    ...
    quest.me = quest.useAs (quest.goblin.goblinName, quest.goblin.id);

    quest.warehouse = quest.useAs ('warehouse', 'warehouse');
    ...
    return watt (function* (quest, next) {
      ...
      try {
        ...
        if (questName === 'delete') {
          ...
          console.log ('Deleting', this._goblinId);
          if (!GOBLINS_REFS[this._goblinId]) {
            throw new Error ('Goblin already deleted:', this.goblinId);
          }
          GOBLINS_REFS[this._goblinId]--;
          console.log ('refcount:', GOBLINS_REFS[this._goblinId]);
          if (GOBLINS_REFS[this._goblinId] === 0) {
            ...
            for (const g in GOBLINS_USES[this.goblinName][this._goblinId]) {
              ...
              if (GOBLINS_REFS[use.id]) {
                if (GOBLINS_REFS[use.id] === 1) {
