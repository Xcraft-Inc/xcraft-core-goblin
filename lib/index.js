  ...
  return {
    ...
    set: watt(function* (name, context, resp, next) {
      for (const id of context.ids) {
        ...
        if (name !== 'warehouse' && name !== 'warehouse-store') {
...
class Goblin {
  ...
  *upserter(quest, payload) {
    ...
    let data = payload.data.delete('private');
    let id = data.get('id');

    if (!id && payload.branch) {
      id = payload.branch;
      data = data.set('id', id);
    }

    if (this.lastUpsertData && this.lastUpsertData.equals(data)) {
      return;
    }

    const _payload = {...payload, data: new Shredder({id})};
    const upserted = yield quest.warehouse.upsert(_payload);
    if (!upserted) {
      return;
    }

    const store = quest.getAPI('warehouse-store');
    yield store.upsert({branch: _payload.branch, data});
    this.lastUpsertData = data;
  }
  ...
  *_doQuest(questName, msg, resp, context, next) {
    ...
    try {
      ...
      if (
        ...
        this.goblinName !== 'warehouse-store' &&
        questName !== 'delete' &&
        !canceled
      ) {
...
module.exports.Elf = require('./elf.js');
