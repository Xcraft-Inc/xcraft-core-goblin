...
class Goblin {
        ...
        quests[questName] = watt(function*(msg, resp, next) {
          ...
          try {
            ...
            if (goblin) {
              ...
              //state don't exist?
              if (!res.data && !goblin.isCreating()) {
                const payload = {
                  branch: id,
                  data: goblin.getState(),
                  parents: msg.data.parent,
                  feeds:
                    (msg.data &&
                      msg.data._goblinFeed &&
        ...
        try {
          ...
          if (questName === 'delete') {
            if (goblin._generationId !== msg.data.generation) {
              console.warn('skipped delete, bad generation');
  ...
  constructor(goblinId, goblinName, logicState, logicHandlers, goblinConfig) {
    ...
    $('dispatch-quest', this.questEmitter)
      .map(({goblin, questName, quest, dispatch, cmdMode}) =>
        watt(function*(next) {
          if (inDeleteState) {
