    ...
    return {
      status: (msg, resp) => {
        ...
        Object.keys (GOBLINS).forEach (name => {
          ...
          if (GOBLINS_REFS[name]) {
            resp.log.info (`  refcount=${GOBLINS_REFS[name].count}`);
          }
        });
  ...
  injectQuestBusHelpers (quest, resp) {
    ...
    quest.createFor = watt (function* (
      ...
      if (skipCreate) {
        ...
        if (GOBLINS_SUBS[id]) {
          Object.keys (GOBLINS_SUBS[id]).forEach (k => {
            const handlers = GOBLINS_SUBS[id][k];
            for (const handler in handlers) {
              quest.goblin.defer (quest.sub (k, handler));
            }
          });
        }
        ...
        const useMore = cGob => {
          ...
          if (GOBLINS_SUBS[cGob._goblinId]) {
            Object.keys (GOBLINS_SUBS[cGob._goblinId]).forEach (k => {
              const handlers = GOBLINS_SUBS[cGob._goblinId][k];
              for (const handler in handlers) {
                quest.goblin.defer (quest.sub (k, handler));
              }
            });
    ...
    quest.sub = function (topic, handler) {
      if (!GOBLINS_SUBS[quest.goblin.id]) {
        GOBLINS_SUBS[quest.goblin.id] = [];
      }
      GOBLINS_SUBS[quest.goblin.id].push ({topic, handler});
        ...
        if (!errThrown) {
          ...
          if (questName === 'delete') {
            if (GOBLINS_REFS[this._goblinId].count === 0) {
              ...
              delete GOBLINS_SUBS[this._goblinId];
            ...
            Object.keys (postRelease).map (k => {
              ...
              delete GOBLINS_SUBS[dGob._goblinId];
