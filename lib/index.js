...
class Goblin {
  ...
  static getQuests(goblinName) {
    ...
    const cryoLocksId = (id) => `_cryoLocks_${id}_`;
      ...
      if (questName === 'create') {
        quests[questName] = watt(function* (msg, resp, next) {
          ...
          try {
            ...
            yield cryoLocks.lock(cryoLocksId(id));
            ...
            cryoLocks.unlock(cryoLocksId(id));
          }
        });
        return;
      }
      ...
      if (questName === 'insertOrCreate' || questName === 'insertOrReplace') {
        quests[questName] = async (msg, resp) => {
          ...
          try {
            ...
            /* lock for the special insertOrCreate / insertOrReplace quest */
            await cryoLocks.lock(cryoLocksId(id));
            ...
            if (!goblin) {
              ...
              const result = await cmd('cryo.hasGoblin', {
                db: ripleyRules.db,
                goblin: `${goblinName}-${id}`,
              });
              const hasGoblin = result?.data?.exist;
              ...
              if (freeze) {
                ...
                if (!hasGoblin && !isServerSide) {
                  const action = createAction(questName, msg.data.state, {id});
