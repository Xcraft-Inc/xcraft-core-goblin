...
$('quest', questEmitter).each(quest => {
  if (quest.caller === undefined) {
    console.log(
      `SCHEDULING: unknow caller-> ${quest.goblin.id}.${
        quest.questName
      } RUN-IN-QUEUE`
    );
    quest.goblin.questEmitter.emit('run-in-queue', quest);
  ...
  if (
    ...
    console.log(
      `SCHEDULING: ${quest.caller}.${quest.callerQuest} -> ${quest.goblin.id}.${
        quest.questName
      } RUN-IN-CREATE`
    );
    quest.goblin.questEmitter.emit('run-in-create', quest);
  } else {
    console.log(
      `SCHEDULING: ${quest.caller}.${quest.callerQuest} -> ${quest.goblin.id}.${
        quest.questName
...
class Goblin {
  ...
  static getQuests(goblinName) {
    ...
    Object.keys(QUESTS[goblinName]).forEach(questName => {
      ...
      const questDelete = (msg, resp, goblin) => {
        if (!msg.data) {
          throw new Error('hum.... brainfuck case....');
        }
        if (
  ...
  constructor(goblinId, goblinName, logicState, logicHandlers, goblinConfig) {
    ...
    const parallelRuns = $('run-in-queue', this.questEmitter)
      .tap(({goblin, questName}) => {
        console.log(`running ${goblin.id}.${questName} from queue`);
      })
      ...
      .tap(({goblin, questName}) =>
        console.log(`running ${goblin.id}.${questName} from queue [DONE]`)
      )
      .stopOnError(err => console.warn(err));

    if (goblinName !== goblinId) {
      const apiRuns = parallelRuns.fork();
      $('run-in-create', this.questEmitter)
        .tap(({goblin, questName}) => {
          console.log(`running ${goblin.id}.${questName} in create`);
        })
        .map(({goblin, questName, quest, dispatch, schedulingMode}) => n => {
          const questDispatcher = function(type, payload = {}, error = false) {
            const action = createAction(type, payload, error);
        ...
        .parallel(Number.MAX_VALUE)
        .tap(({goblin, questName}) =>
          console.log(`running  ${goblin.id}.${questName} in create [DONE]`)
        )
        .stopOnError(err => console.warn(err))
        .done(() => console.log('ended'));
    } else {
      parallelRuns.done(() => console.log('ended'));
    }
