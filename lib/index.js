...
class Goblin {
  ...
  constructor(
    ...
    const parallelRunner = $('parallel', queue)
      .map(({goblin, questName, quest, dispatch, done}) => next => {
        Goblin._questDispatch(goblin, questName, quest, dispatch, done, next);
      })
      .nfcall([])
    ...
    const nextDelete = call => {
      deletePromise = new Promise(resolve => call(resolve));
      deletePromise.then(() => {
        deletePromise = null;
      });
    };

    const nextParallel = (call, promises) => {
    ...
    let inDeleteState = false;
    $('dispatch-quest', this.questEmitter)
      .map(({goblin, questName, quest, dispatch, cmdMode, msg, resp}) =>
        watt(function*(next) {
          if (inDeleteState) {
            console.warn(`${questName} running when a delete as started`);
          }
          switch (cmdMode) {
            case 'delete':
              {
                if (msg.data.generation !== goblin._generationId) {
                  resp.events.send(`${goblinName}.delete.${msg.id}.finished`);
                  return;
                }
                inDeleteState = true;
                //console.log('DELETING ', goblin.id);
                ...
                yield next.sync();
                //console.log('DELETE ', msg.data.id, ' [DONE]');
              }
              break;
            case 'recreate':
              //console.log('RECREATE ', goblin.id);
              if (deletePromise) {
                yield deletePromise;
              } else {
                resp.events.send(
                  `${goblinName}.create.${msg.id}.finished`,
                  goblin.id
                );
                //console.log('RECREATE ', goblin.id, '[DONE]');
                break;
              }
              ...
              if (questName === 'create') {
                ...
                nextParallel(
                    ...
                    queue.emit('create', {
                      ...
                      //console.log('CREATE ', goblin.id, '[DONE]');
                      // done(err, r);
                      //},
                    }),
                  createPromises
