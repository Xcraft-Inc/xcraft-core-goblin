...
class Goblin {
  ...
  deferCreateEvent(msgId) {
    this._createMsgIdList.push(msgId);
  }
  ...
  doQuest(questName, msg, resp) {
    ...
    return watt(function*(context, next) {
      ...
      if (msg.data._goblinRecreate) {
         ...
         * the first `create` is finished. Excepted if the call comes from
         * our own instance (jonny's case).
         */
        if (
          msg.data._goblinCaller !== this._goblinId &&
          this._runningQuests.create > 1
        ) {
          this.deferCreateEvent(msg.id);
          return;
        }
      ...
      try {
        ...
        if (questName === 'create') {
          this._createMsgIdList.push(msg.id);
        }
        ...
        if (!errThrown) {
          ...
          try {
            if (questName === 'create') {
              this._createMsgIdList.forEach(msgId =>
                resp.events.send(
                  `${this.goblinName}.${questName}.${msgId}.finished`,
                  result
                )
              );
            } else {
        ...
        if (errThrown) {
          ...
          if (questName === 'create') {
            this._createMsgIdList.forEach(msgId =>
              resp.events.send(
                `${this.goblinName}.${questName}.${msgId}.error`,
                {
                  code: ex.code,
                  message: ex.message,
                  stack: ex.stack,
