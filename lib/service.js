...
Goblin.registerQuest(goblinName, 'ripleyCheckForCommitId', async function (
  ...
  for (const commitId of commitIds) {
     ...
     */
    // TODO: check how many persists must be returned with this commitId,
    //       If the number of persists is too high, stop immediatly and returns
    //       false because a clean bootstrap is needed for this database.
    //       Too much persists with the standard sync is a bad idea because
    //       it's using a sync loop with a very large array, see:
    //       rangeOfActions in ripleyServer
