...
Goblin.registerQuest(goblinName, 'ripleyServer', async function (
  ...
  if (toCommitId && fromCommitId !== toCommitId) {
    ...
    quest.log.dbg(
      `[${logPrefixId}] retrieve from ${db}, range ${fromHex}:${toHex} server's actions`
    );
...
Goblin.registerQuest(goblinName, 'ripleyCheckForCommitId', async function (
  ...
  for (const commitId of commitIds) {
    ...
    if (await cryo.hasCommitId({db, commitId})) {
       ...
       */
      const count = await cryo.countNewRowsFrom({db, commitId});
...
Goblin.registerQuest(goblinName, 'ripleyClient', async function (quest, db) {
  ...
  const {persisted, xcraftStream, routingKey} = results;
  newCommitId = results.newCommitId;

  if (xcraftStream && routingKey) {
    const stream = new RipleyWriter(quest, db, horde, persisted);
    const streamer = promisify(xcraftStream.streamer);
    await streamer(routingKey, stream, null);
  }

  if (persisted.length) {
    await applyPersisted(quest, db, horde, persisted);
  }
