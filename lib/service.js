...
Goblin.registerQuest(goblinName, 'ripleyServer', async function (
  ...
  quest.log.dbg(
    `[${logPrefixId}] begin for ${db} with ${actions.length} actions`
  );
  ...
  quest.log.dbg(`[${logPrefixId}] begin for ${db}`);
  ...
  if (toCommitId && fromCommitId !== toCommitId) {
    ...
    quest.log.dbg(
      `[${logPrefixId}] ripley for ${db}, range ${fromCommitId}:${toCommitId} with ${rangeOfActions.length} actions`
    );
...
Goblin.registerQuest(goblinName, 'ripleyClient', async function (quest, db) {
  ...
  for (let i = 0; i < persisted.length; i += step) {
    quest.log.dbg(
      `ripley for ${db}, replay actions ${i}:${i + step - 1} (total ${
        persisted.length
      })`
    );
