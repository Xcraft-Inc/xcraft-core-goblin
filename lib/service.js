...
Goblin.registerQuest(goblinName, 'ripleyServer', async function (
  ...
  if (toCommitId && fromCommitId !== toCommitId) {
    ...
    const unsub = quest.sub.local(
      `*::cryo.range.<${id}>.chunk`,
      (err, {msg}) => {
        const actions = msg.data;
        rangeLength += actions.length;
    ...
    quest.log.dbg(
      `[${logPrefixId}] retrieve from ${db}, range ${fromHex}:${toHex} of ${rangeLength} server's actions`
    );

    /* We use the concat function in order to merge our both arrays */
    persisted = _persisted.concat(persisted);
  }
...
Goblin.registerQuest(goblinName, 'ripleyClient', async function (quest, db) {
  ...
  if (!commitCnt) {
    throw new Error(`Old servers without commit counter are not supported`);
  }
