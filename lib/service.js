...
Goblin.registerQuest(goblinName, 'ripley', function* (quest, actions) {
  const systemDesktop = `system@${quest.uuidV4()}`;
  const goblinIds = [];
  try {
    for (const actionStr of actions) {
      const action = JSON.parse(actionStr);
      const goblinId = action.meta.id;
      goblinIds.push(goblinId);
      const goblin = yield quest.create(goblinId, {
        id: goblinId,
    ...
    // TODO: generate persist action when the batch of each goblin is finished
  } finally {
    yield quest.kill(goblinIds, quest.goblin.id, systemDesktop);
  }
});
