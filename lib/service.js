...
Goblin.registerQuest(goblinName, 'ripleyServer', async function (
  ...
  const logPrefixId = `${user}/${msgId}/${lastClientCommitId?.substring(0, 8)}`;
  ...
  if (lastClientCommitId) {
    hasCommitId = await cryo.hasCommitId({db, commitId: lastClientCommitId});
  }
...
Goblin.registerQuest(goblinName, 'ripleyCheckBeforeSync', async function (
  ...
  try {
    ...
    lastCommitId = await cryo.getLastCommitId({db});
    if (!lastCommitId?.commitId) {
      return true;
    }
  } catch (ex) {
...
Goblin.registerQuest(goblinName, 'ripleyClient', async function (quest, db) {
  ...
  if (zeroActions.length) {
    // FIXME: don't return the persisted anymore
