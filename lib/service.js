...
Goblin.registerQuest(goblinName, 'ripleyClient', async function (quest, db) {
  ...
  const _persisted = Object.entries(persisted);

  const step = 100;
  for (let i = 0; i < _persisted.length; i += step) {
    quest.log.dbg(
      `ripleyClient replay actions ${i}:${i + step - 1} (total ${
        _persisted.length
      })`
    );
    const actions = Object.fromEntries(_persisted.slice(i, i + step));
