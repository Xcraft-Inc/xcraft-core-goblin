...
Goblin.registerQuest(goblinName, 'ripleyServer', async function (
  ...
  const goblins = [];
  const goblinsMap = {};
  const logPrefixId = lastClientCommitId.substring(0, 8);

  const cryo = quest.getAPI('cryo');

  quest.log.dbg(
    `[${logPrefixId}] ripleyServer begin for ${db} with ${actions.length} actions`
  );

  // (4) (9)
  await cryo.immediate({db});
  quest.defer(async () => {
    quest.log.dbg(`[${logPrefixId}] ripleyServer commit for ${db} `);
    await cryo.commit({db});
  });

  quest.log.dbg(`[${logPrefixId}] ripleyServer locked for ${db}`);
  ...
  if (toCommitId && fromCommitId !== toCommitId) {
    ...
    quest.log.dbg(
      `[${logPrefixId}] ripleyServer for ${db}, range ${fromCommitId}:${toCommitId} with ${rangeOfActions.length} actions`
    );
...
Goblin.registerQuest(goblinName, '_ripleyApplyPersisted', async function (
  ...
  for (const {id, data} of persisted) {
    const localGoblin = await quest.create(id, {
      id,
      desktopId: systemDesktop,
    });
...
Goblin.registerQuest(goblinName, 'ripleyClient', async function (quest, db) {
   ...
   */
  const step = 100;
  for (let i = 0; i < persisted.length; i += step) {
    quest.log.dbg(
      `ripleyClient for ${db}, replay actions ${i}:${i + step - 1} (total ${
        persisted.length
      })`
    );
    const actions = persisted.slice(i, i + step);
...
Goblin.registerQuest(goblinName, 'tryShutdown', function* (quest, wait) {
  ...
  const databases = Object.keys(thinkData);
  if (wait && databases.length) {
