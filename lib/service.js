...
Goblin.registerQuest(goblinName, 'ripley', function* (quest, actions) {
  ...
  try {
    ...
    for (const {goblin, actions} of Object.values(goblins)) {
      const hash = yield goblin.$4ellen({
        actions,
        _xcraftRPC: true,
      });
      hashes[goblin.id] = hash;
    }

    /* Check if this hash is new and if it's the case get the whole
     * get the whole pesist action and replay this one on the client
     * side (the new truth).
     */
    // TODO
  } finally {
