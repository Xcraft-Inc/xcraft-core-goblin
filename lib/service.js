...
const {CoalescingExecutor} = require('xcraft-core-utils/lib/locks.js');
...
Goblin.registerQuest(goblinName, 'ripleyCheckForCommitId', async function (
  ...
  for (const commitId of commitIds) {
    ...
    if (await cryo.hasCommitId({db, commitId})) {
      ...
      const check = count > 20000 ? false : true;
      if (!check) {
        quest.log.dbg(
          `There are a lot of actions to sync for ${db} when using ${commitId}`
        );
      }
      return {check, count};
    }
...
const ripleyClient = async (quest, db) => {
  const cryo = quest.getAPI('cryo');
...
Goblin.registerQuest(goblinName, 'ripleyClient', async function (quest, db) {
  ...
  /* Run for the first and the last calls of ripleyClient for a database.
   * The intermediates calls are skipped (useless). Note that there is already
   * a debouncer on top of this executor. The purpose here is mainly to limit
   * useless sync when the delay with the server is larger than the debouncer
   * client delay.
   */
  const executor = quest.goblin.getX(
    `ripley.barriers.${db}`,
    new CoalescingExecutor()
  );
  await executor.run(async () => ripleyClient(quest, db));
});
