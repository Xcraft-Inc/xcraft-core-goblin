...
Goblin.registerQuest(goblinName, '_init', function* (quest, $msg, next) {
  ...
  const {appId, variantId, appVersion} = xHost;
  ...
  const {graffiti} = require('xcraft-core-utils/lib/log.js');
...
• appId      : ${appId}
• variantId  : ${variantId ? variantId : 'N/A'}
• appVersion : v${appVersion}
• tribe      : ${tribe ? tribe : 'N/A'}

%sWoooah, ready to deliver ${goblins.size} pointy features!
`;
  quest.log.dbg(
    welcome,
    yield graffiti('goblin-core', next),
    goblinConfig.actionsSync?.enable ? `Actions sync party started\n` : ''
  );
});
 ...
 * be placed before these persists.
 */
async function ripleyServer(quest, db, actions, commitIds, userId, $msg) {
  ...
  if (toCommitId && fromCommitId !== toCommitId) {
    const {count} = await quest.me.ripleyCheckForCommitId({
      db,
      commitIds: [fromCommitId],
    });

    /* Commit, then getPersistFromRange can read the whole new actions
     * and not just the actions before the immediate transaction. It's necessary
     * since that the queries are handled by an other SQLite connection
     * via a worker thread. The selects are no longer using the immediate
     * transaction initiated by ripleyServer.
     */
    quest.log.info(`[${logPrefixId}] commit for ${db} `);
    await cryo.commit({db});
    commitDone = true;

    const {xcraftStream} = await cryo.getPersistFromRange({
      db,
      fromCommitId,
      toCommitId,
    ...
    quest.log.dbg(
      `[${logPrefixId}] retrieve from ${db}, range ${fromHex}:${toHex} of ${count} server's actions`
    );
    ...
    return {
      newCommitId,
      persisted,
      xcraftStream: xcraftStream.getStream(),
      routingKey,
      count,
    };
  }
...
async function ripleyPersistFromZero(quest, db, goblinIds) {
  const cryo = quest.getAPI('cryo');
  return await cryo.hasActions({db, goblinIds});
}

async function ripleyGetAllPersist(quest, db) {
  const cryo = quest.getAPI('cryo');
  const result = await cryo.getAllPersist({db});
  if (result.xcraftStream) {
    result.xcraftStream = result.xcraftStream.getStream();
  }
  return result;
}
...
async function ripleyCheckForCommitId(quest, db, commitIds) {
  const cryo = quest.getAPI('cryo');
  ...
  if (!lastCommitId?.commitId) {
    return {
      check: true,
      count: 0,
    }; /* The client will populate the server side */
  }
  ...
  for (const commitId of commitIds) {
    ...
    if (await cryo.hasCommitId({db, commitId})) {
      ...
      const limit = goblinConfig.actionsSync.bootstrapLimit;

      /* Check how many ~persists (rows) must be returned with this commitId,
       * If the number of rows is too high, stop immediatly and returns false
       * because a clean bootstrap is needed for this database.
       * Too much persists with the standard sync is a bad idea because it's
       * using a sync loop with a very large array.
       * see: rangeOfActions in ripleyServer
       */
      const count = await cryo.countNewPersistsFrom({db, commitId});
      const check = count > limit ? false : true;
      if (!check) {
        quest.log.dbg(
          `There are a lot of actions to sync for ${db} when using ${commitId}; more than ${limit}`
        );
      }
      return {
        check,
  ...
  const count = await cryo.countPersistsFrom({db});
  return {check: false, count};
}
...
Goblin.registerQuest(goblinName, 'ripleyCheckBeforeSync', async function (
  ...
  try {
    ...
    return {passed: false}; /* maybe the database is corrupted */
  }

  let passed = false;
  let commitId;
  try {
    /* Check if the server has these commitIds too. */
    const results = await quest.cmd('goblin.ripleyCheckForCommitId', {
      _xcraftRPC: true,
      db,
      commitIds,
    });
  ...
  return {passed, commitId};
});

async function ripleyClient(quest, db) {
  const {getMutex} = require('xcraft-core-utils/lib/locks.js');
  const syncLock = getMutex;
  const cryo = quest.getAPI('cryo');
  ...
  if (quest.goblin.getX(SHUTTING_DOWN_KEY)) {
  ...
  const {stagedActions, commitIds} = await quest.me._ripleyPrepareSync({db});
  ...
  /*`persisted`: actions corresponding to `stagedActions` */
  const {persisted, xcraftStream, routingKey, count} = results;
  newCommitId = results.newCommitId;

  if (xcraftStream && routingKey) {
    let stream = new RipleyWriter(quest, db, horde, persisted, count);
    quest.defer(() => {
      if (stream) {
        stream.destroy();
        stream = null;
    ...
    await streamer(routingKey, stream, null);
    stream.destroy();
    stream = null;
  }

  if (persisted.length) {
    await applyPersisted(quest, db, persisted);
  }
...
Goblin.registerQuest(goblinName, 'tryShutdown', function* (quest, wait, next) {
  ...
  quest.goblin.setX(SHUTTING_DOWN_KEY, true);
...
/* prettier-ignore */
if (goblinConfig.actionsSync.isServer) {
  Goblin.registerQuest(goblinName, 'ripleyServer', ripleyServer);
  Goblin.registerQuest(goblinName, 'ripleyPersistFromZero', ripleyPersistFromZero);
  Goblin.registerQuest(goblinName, 'ripleyGetAllPersist', ripleyGetAllPersist);
  Goblin.registerQuest(goblinName, 'ripleyCheckForCommitId', ripleyCheckForCommitId);
} else {
  Goblin.registerQuest(goblinName, 'ripleyClient', ripleyClient);
}
