 ...
 */
async function wrapForSyncing(quest, db, handler, progress) {
  let timeout;
  const DELAY = 1000;
  ...
  try {
    ...
    quest.defer(() => {
      ...
      setTimeout(() => sendSyncing(quest, db, horde, false), DELAY, progress);
    });
...
Goblin.registerQuest(goblinName, 'ripleyClient', async function (quest, db) {
  ...
  for (let i = 0; i < persisted.length; i += step) {
    ...
    quest.log.dbg(
      `ripley for ${db}, replayed actions ${i}:${i + step - 1} (total ${
        persisted.length
      }) in ${(end - start) / 1_000_000n}ms`
    );
