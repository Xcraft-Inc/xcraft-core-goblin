...
Goblin.registerQuest(goblinName, '_init', function* (quest, $msg, next) {
  ...
  if (appArgs['disable-actions-sync']) {
    syncEnabled = false;
  }
  if (syncEnabled) {
...
Goblin.registerQuest(goblinName, 'ripleyOne', function* (quest, db, goblin) {
  ...
  /* FIXME: atomicity problem here
   *
   * 1. warehouse.has (async)
   * IF TRUE
   * 2.a create (async)
   * 2.b persist (async)
   * IF FALSE
   * 2.a cryo.freeze (async)
   *
   * There are possible races (create while freeze; create while
   * warehouse.has, etc.). When a sync operation is done for a
   * specific goblinId, this goblin must be locked. It should be
   * possible to handle this lock via the Elf's core internal.
   */
...
Goblin.registerQuest(goblinName, 'ripleyServer', function* (
  ...
  // (4)
  yield quest.cmd('cryo.immediate', {db});
  // (9)
  quest.defer(async () => await quest.cmd('cryo.commit', {db}));

  // (5)
  const lastCommitId = yield quest.cmd('cryo.getLastCommitId', {db});
  ...
  try {
    ...
    // (7)
    const newCommitId = commitUpdated ? quest.uuidV4() : null;

    for (const p of Object.values(persisted)) {
      p.commitId = newCommitId;
    }

    yield quest.cmd('cryo.ripleyCommit', {
      db,
      newCommitId,
      goblinIds: Object.keys(persisted).map(
        (goblinId) => `${goblinId.split('@', 1)[0]}-${goblinId}`
      ),
    });

    // (8)
    const rangeOfActions = yield quest.cmd('cryo.getPersistFromRange', {
      db,
...
Goblin.registerQuest(goblinName, 'ripleyClient', function* (quest, db) {
  ...
  // (1) and (2)
  const {stagedActions, lastCommitId} = yield quest.cmd('cryo.getDataForSync', {
    db,
  });
  const {newCommitId, persisted} = yield quest.cmd('goblin.ripleyServer', {
    _xcraftRPC: true,
    db,
  ...
  try {
    ...
    for (const goblinId of Object.keys(persisted)) {
      ...
      // TODO: store the persisted[goblinId].hash as last common hash for this goblin
    }

    // (12)
    if (newCommitId && stagedActions?.length) {
      yield quest.cmd('cryo.updateActionsAfterSync', {
        db,
