...
Goblin.registerQuest(goblinName, '_ripleyPrepareSync', async function (
  ...
  return {stagedActions, lastCommitId, rows};
});
...
Goblin.registerQuest(goblinName, 'ripleyClient', async function (quest, db) {
  ...
  quest.defer(await syncLock.unlock(`ripleyClient-${db}`));

  const {
    stagedActions,
    lastCommitId,
    rows,
  } = await quest.me._ripleyPrepareSync({db});
   ...
   * for these actions and everything is successful.
   */
