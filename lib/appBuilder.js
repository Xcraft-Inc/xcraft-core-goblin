...
module.exports = (appId, config) => {
  ...
  Goblin.registerQuest(appId, 'boot', function* (quest, $msg, next) {
    ...
    if (config.quests.boot) {
      ...
      const reflect = require('xcraft-core-utils/lib/reflect.js');
      ...
      const {isGenerator, isAsync} = require('xcraft-core-utils/lib/js.js');
      //inject desktopId in message data
      $msg.data.desktopId = desktopId;
      if (isGenerator(_boot)) {
        yield* boot(quest, $msg, next);
      } else if (isAsync(_boot)) {
        yield boot(quest, $msg);
      } else {
        boot(quest, $msg);
      }
    }
  ...
  if (config.useWorkshop) {
    Goblin.registerQuest(appId, 'getMandate', function (quest) {
      const {profile} = require('xcraft-core-etc')().load(`goblin-${appId}`);
      if (profile && profile.mandate) {
