...
module.exports = (appId, config) => {
  ...
  const logicHandlers = {
    'init': (state, action) => {
      return state.set('host', action.get('host'));
    },
    'open-desktop': (state) => {
  ...
  Goblin.registerQuest(appId, 'boot', function* (quest) {
    ...
    if (config.useWorkshop) {
      ...
      try {
        ...
        throw new Error(
          'Fatal error occured during polypheme system storage initialization, check your storages services!'
        );
      }
    }
    console.log(
  ...
  Goblin.registerQuest(appId, 'open-desktop', function* (
    ...
    if (desktopId === labId) {
      desktopId = `desktop@${configuration.mandate}@${username}`;
    }
    // CREATE A DESKTOP
    ...
    let currentContext = yield desk.getCurrentContext();
    ...
    yield desk.navToContext({contextId: currentContext});

    const exSession = quest.goblin.getX(`${desktopId}.session`);
    if (!exSession) {
      quest.goblin.setX(
        `${desktopId}.session`,
        quest.sub(`*::${desktopId}.session.closed`, function* () {
          yield quest.me.closeDesktop({
            clientSessionId,
            desktopId,
  ...
  Goblin.registerQuest(appId, 'close-desktop', function (
    quest,
    desktopId,
    isSessionClosed
  ) {
    ...
    if (isSessionClosed) {
      quest.release(desktopId);
      quest.evt(`${desktopId}.closed`);
      return;
    }

    quest.release(desktopId);
    quest.evt('desktop.leaved', {desktopId});
  });
