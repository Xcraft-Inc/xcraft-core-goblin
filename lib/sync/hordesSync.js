const goblinConfig = require('xcraft-core-etc')().load('xcraft-core-goblin');
const Goblin = require('xcraft-core-goblin');
const xBus = require('xcraft-core-bus');
const xHorde = require('xcraft-core-horde');
const Command = require('xcraft-core-busclient/lib/command.js');
const {setTimeout: setTimeoutAsync} = require('node:timers/promises');
const {locks} = require('xcraft-core-utils');

class HordesSync {
  #initialized = false;
  #initializing = false;
  #socketLost = true;
  #socketLag = false;
  #resp;
  #bootSyncing = {};
  #bootLock = locks.getMutex;

  constructor() {
    const busClient = require('xcraft-core-busclient').getGlobal();

    this.#resp = busClient.newResponse('elf', 'token');
    this.#resp.events.subscribe('greathall::<perf>', (msg) =>
      this.#monitorPerf(this.#resp, msg.data)
    );
  }

  get initialized() {
    return this.#initialized;
  }

  #logResults(results) {
    const max = results.reduce(
      (max, {db}) => (db.length > max ? db.length + 1 : max),
      0
    );
    this.#resp.log.dbg(`Bootstrap results:`);
    for (const {db, empty, exists} of results) {
      const level = !empty && exists ? 'info' : 'warn';
      this.#resp.log[level](
        `→ ${db}${new Array(max - db.length).join(
          ' '
        )} empty=${empty} exists=${exists}`
      );
    }
  }

  #monitorPerf(resp, data) {
    const {
      horde, // appId
      delta, // ms now <-> last connect
      lag, // true delta > 1s
      // overlay, // true delta > 10s need display overlay
      noSocket, // true if no more socket available
      syncing,
    } = data;

    /* skip because it's not related to socket changes */
    if (syncing) {
      return;
    }

    const {topology} = require('xcraft-core-etc')().load('xcraft-core-horde');
    const isSyncServer = !topology?.[horde]?.noSync;

    resp.log.dbg(`H=${horde} d=${delta} lag=${lag} noSocket=${noSocket}`);

    if (!isSyncServer) {
      if (!noSocket && lag && delta >= 30000) {
        Command.abortAll(horde, 'timeout of 30s');
      }
      return;
    }

    if (noSocket) {
      this.#socketLost = true;
    } else {
      if (lag && delta >= 30000) {
        this.#socketLag = true;
        Command.abortAll(horde, 'timeout of 30s');
      } else {
        this.#socketLag = false;
      }
    }

    if (!noSocket && (this.#socketLost || this.#socketLost === undefined)) {
      this.#socketLost = false;
      this._syncAll(true);
    }
  }

  _syncAll() {
    if (!this.#initialized) {
      this.boot(); /* fire and forget */
      return;
    }

    for (const db of Goblin.getAllRipleyDB()) {
      /* fire and forget */
      this.sync(db);
    }
  }

  _sendSyncing(db, status, progress = undefined) {
    try {
      const routingKey = xBus
        .getCommander()
        .getRoutingKeyFromId('goblin.ripleyServer', null, true);
      Object.assign(this.#bootSyncing, {[db]: {sync: status, progress}});
      this.#resp.events.send('greathall::<perf>', {
        horde: routingKey,
        syncing: this.#bootSyncing,
      });
    } catch (ex) {
      if (ex.code === 'CMD_NOT_AVAILABLE') {
        this.#resp.log.warn(`Skip syncing for ${db}: ` + ex.message);
      } else {
        this.#resp.log.err(ex.stack || ex.message || ex);
      }
    }
  }

  async #bootstrapActions(db, rename) {
    /* Retrieve all persist actions with an xcraftStream */
    const {
      xcraftStream,
      routingKey,
      count,
    } = await this.#resp.command.sendAsync('goblin.ripleyGetAllPersist', {
      _xcraftRPC: true,
      db,
    });
    if (!xcraftStream) {
      if (count > 0) {
        this.#resp.log.err(
          `No xcraftStream for ${db} but the count is ${count}`
        );
      }
      return;
    }

    const {streamId} = xcraftStream;

    const unsub = this.#resp.events.subscribe(
      '*::cryo.bootstrapActions.progress',
      ({data}) => this._sendSyncing(db, true, data)
    );

    try {
      /* Forward the xcraftStream identifications to the real destination */
      await this.#resp.command.sendAsync('cryo.bootstrapActions', {
        db,
        streamId,
        routingKey,
        rename,
        count,
      });
    } finally {
      if (unsub) {
        unsub();
      }
    }
  }

  async _bootstrap(db) {
    let empty = true;
    let exists = false;
    let rename = false;

    try {
      await this.#bootLock.lock(`_bootstrap/${db}`);

      try {
        /* Check if the database exists and if it's the case, check if empty */
        const result = await this.#resp.command.sendAsync('cryo.isEmpty', {db});
        if (result) {
          empty = result.empty;
          exists = result.exists;
        }
      } catch (ex) {
        if (ex.code === 'SQLITE_CORRUPT') {
          empty = true;
          exists = false;
          this.#resp.log.err(
            `Database ${db} is corrupted, we need to bootstrap`
          );
        } else {
          throw ex;
        }
      }

      if (!empty) {
        /* Check if the last commitId is known */
        const results = await this.#resp.command.sendAsync(
          'goblin.ripleyCheckBeforeSync',
          {db, noThrow: true}
        );

        if (results?.passed) {
          if (results.commitId) {
            await this.#resp.command.sendAsync('goblin.ripleyClient', {db});
          }
          return {db, exists, empty, passed: true};
        }

        /* Rename the database and bootstrap a new one */
        this.#resp.log.warn(
          `The last commitId for ${db} is not known by the server then bootstrap again`
        );
        rename = true;
      }

      this._sendSyncing(db, true);
      await this.#bootstrapActions(db, rename);
      return {db, passed: true};
    } catch (ex) {
      if (ex.code === 'CMD_NOT_AVAILABLE') {
        this.#resp.log.warn(`Skip bootstrap for ${db}: ` + ex.message);
      } else {
        this.#resp.log.err(ex.stack || ex.message || ex);
      }
      return {db, exists, empty};
    } finally {
      this._sendSyncing(db, false);
      this.#bootLock.unlock(`_bootstrap/${db}`);
    }
  }

  _boot(db) {
    if (goblinConfig.actionsSync?.excludeDB?.includes(db)) {
      this._sendSyncing(db, false);
      return;
    }

    this._sendSyncing(db, true);
    return () => this._bootstrap(db);
  }

  async boot() {
    if (this.#initialized || this.#initializing) {
      return true;
    }

    this.#initializing = true;
    xHorde.setMaxLagDeltaTime(60000 * 4); /* 4 minutes */

    try {
      /* Give a chance to the client to connect to the server */
      await xHorde.waitAutoload();

      while (true) {
        const isConnected = xHorde
          .getSlaves()
          .filter((routingKey) => xHorde.hasSyncing(routingKey))
          .every((routingKey) => {
            const slave = xHorde.getSlave(routingKey);
            return slave === -1 ? false : slave.isConnected;
          });

        const boots = [];
        for (const db of Goblin.getAllRipleyDB()) {
          const boot = this._boot(db);
          if (boot) {
            boots.push(boot);
          }
        }

        const results = [];
        for (const boot of boots) {
          results.push(await boot());
        }

        if (
          results.some((status) => status?.exists === false) ||
          /* When connected, wait at least that all database are ready */
          (isConnected && results.some((status) => !status?.passed))
        ) {
          /* Bootstrap mandatory, retry */
          this.#resp.log.dbg('The bootstrap is mandatory, retry');
          await setTimeoutAsync(1000);
          continue;
        }

        /* Bootstrapped... */
        this.#logResults(results);
        break;
      }
    } finally {
      xHorde.setMaxLagDeltaTime();
      this.#initializing = false;
    }

    this.#initialized = true;
    this.#resp.events.send('goblin.hordesSync-initialized');

    for (const db of Goblin.getAllRipleyDB()) {
      this.sync(db); /* fire and forget */
    }
    return true;
  }

  async sync(db) {
    if (
      goblinConfig.actionsSync?.excludeDB?.includes(db) ||
      this.#socketLost ||
      this.#socketLag ||
      !this.#initialized
    ) {
      return;
    }

    try {
      await this.#resp.command.sendAsync('goblin.ripleyClient', {db});
      return true;
    } catch (ex) {
      this.#resp.log.warn(ex.message || ex.stack || ex);
      return false;
    }
  }
}

/** @type {HordesSync} */
let hordesSync;

module.exports = () => {
  if (!hordesSync) {
    hordesSync = new HordesSync();
  }
  return hordesSync;
};
