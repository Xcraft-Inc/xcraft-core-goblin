...
class HordesSync {
  ...
  constructor() {
    ...
    this.#resp = busClient.newResponse('elf', 'greathall');
  ...
  async _bootstrap(db) {
    ...
    try {
      ...
      this.#resp.log.warn(`[${db}]:`, ex.stack || ex.message || ex);
  ...
  async boot() {
    ...
    const limit = pLimit(4); /* 4 databases at a time */
