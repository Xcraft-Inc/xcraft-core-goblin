...
module.exports = (busConfig) => {
  ...
  const {readJSONSync, mkdirpSync, removeSync} = require('fs-extra');
  ...
  if (!usersCacheDir) {
    ...
    if (appId === appMasterId) {
      removeSync(usersPath);
  ...
  const usersDB = new ShieldUsers(usersCacheDir);
  const getUserInCache = (id) => usersDB.get(id);
  const saveUser = (id, data) => usersDB.save(id, data);
  const deleteUser = (id) => usersDB.delete(id);
