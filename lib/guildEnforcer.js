...
module.exports = (busConfig) => {
  ...
  const getUserInCache = (id) => {
    return readJSONSync(path.join(usersCacheDir, id), {throws: false});
  };
  const saveUser = (id, data) => {
    writeJSONSync(path.join(usersCacheDir, id), data);
  };
  const deleteUser = (id) => {
    removeSync(path.join(usersCacheDir, id));
  };
  ...
  enforcer.getUser = (userId) => {
    ...
    userInCache.canDo = (cmd) => {
      const isBlocked = predictions[cmd]
        ? predictions[cmd][userInCache.rank]
        : true;
      return !isBlocked;
    };
