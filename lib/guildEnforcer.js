...
module.exports = (busConfig, goblinConfig) => {
  ...
  const shieldUpsert = (id, login, rank, createAt, member) => {
    if (!enforcer.enforce) {
      return;
    }
  ...
  const shieldDelete = (id, login, rank, createAt, member) => {
    delete users[id];
  };

  const usersDB = enableCache
    ? new ShieldUsers(usersCacheDir, shieldUpsert, shieldUpsert, shieldDelete)
    : null;
  ...
  enforcer.registerUser = (userInfos) => {
    const {id, rank, login} = userInfos;
    const user = {
      id,
      login: login,
      rank,
